...
| **健康** | `kernel/src/health/{mod,pagetable,spare}` | **out-of-path** 开机验收，boot 时经 `health::run()` 一次性调用 | `expect!` → panic → halt |
...
- **护栏语义**：hook 恒编译、单行调用（事件入口 `on_alloc/on_free/…`，asm/毒化/记账收在 fence 内部），release 空体零开销；深度检查（`banker`/`ledger`/`audit`）debug 构建生效、release 编译为空。
