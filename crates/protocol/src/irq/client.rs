...
impl Line {
    ...
    /// 内建核对来源）→ 解回执 → **放下回信孔**。
    ///
    /// 推送是**无界的**（槽满即等，见 `HolePie::push`）：驱动一定会看到这条报文。
    /// 但"看到"不是"办到"——回执才是。
    ///
    /// **这一处是请求-回信这条链上唯一保留无界推的地方**：登记不能"算了"——少一条就是
    /// 一条线没有属主，而调用方拿到 `Busy` 也只能原样重试。`dispatch`/`doom`/`uart`/
    /// `console` 四家的推都是 [`crate::session::push_within`]，别照这一处的样子改它们。
    /// （握手通道上另有几处无界推——`startup::send`、`Refer::push`、`console::open::grant`
    /// ——那是另一笔账，不在上面这句里。）
    ///
