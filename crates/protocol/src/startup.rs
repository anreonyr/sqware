//! startup — 启动期握手：父域**开上行孔**、子域**靠泊**、两条报文。
//!
//! **本模块是一条协议，不是机制**：父域与子域各说一句、线形定死，与 `dispatch` /
//! `console` 那几条同一类。它住 `protocol` 而不是 `runtime::core` 就是这一句理由——
//! 「引荐」那一对（`Refer` / `Referred`）早就以同一条理由搬去了
//! [`crate::dispatch::control`]；`runtime` 那边则因此第一次真的"不认识任何协议"。
//!
...
//! 是两条不同的孔。
//!
//! **靠泊的两半是同一份契约**：父侧 [`berth`] 授的 `R|W|VEST` 与子侧 [`moor`] 认的
//! `vestor == sire && owner == sire`，说的是同一条"上行孔长什么样"——拆开就没人能在一
//! 处读懂它，故两半同住本模块。
//!
...
///
/// 名字：父域给子域**泊好**这条上行孔（`berth` 给泊位），子域从它 [`moor`] 系上来。
/// `Dock` 那个词归 Pole 的封装（`runtime::core::dock`），故这里不用它。
pub fn berth(child: TaskId) -> EnvResult<HolePie> {
...
pub fn moor() -> EnvResult<HolePie> {
    let sire = runtime::env::task::sire()?.get();
    if sire == 0 {
        return Err(denied());
    }
