...
//!
//! 1. **要再交一次句柄**（[`client::land`](client::land) 头一手就是 [`ship`](call::ship)，
//!    造一份新副本种进持树者表里）；
...
//! 窗格本来就没有行 ⇒ 照样得 `grow`），"比 `land` 简单"是假的；且它的地址只有 (坐标, 名)，
...
//!  3. **没有死亡道**：板那是给编排域监督服务用的（一位服务一条 `gone-<名字>`）。装配者那一侧
//!     也照这一句办——**道只给上板的那几位铸**（`system/main.rs` 那一圈）。树这一侧
//!     不推任何东西给装配者。
