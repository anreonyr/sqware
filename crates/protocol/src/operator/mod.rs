...
//! 译完就按号走：别的原语一律收号。名字在答话那一侧也留着一条（[`Operator::name`] 按号答名），
//! 于是"人念得出的那半"与"机器认得的那半"各有一条路，中间只有 `seek` 这一道闸。
//!
//! # 七条原语
//!
//! | 原语 | 干什么 | 落在一枚条目上 |
...
//! | [`Operator::seek`] | 译 | 把一条路**译成那一枚号**（名字只到这一格） |
...
//!
//! **七条都在线上**：四条一格状态的（`land` / `part` / `find` / `trim`）与三条答数据的
//! （`list` 答一串号、`name` 答一枚名字、`seek` 答一枚号）。**答话那一侧有四种帧形**，各有各的
//! 上界（见 [`call::REPLY_MAX`] 那一段）；`list` / `name` / `seek` 三家分家，就是"名与号分开"
//! 那一条落地：**名字只到 `seek` 这一格**。
