//! operator 的**帧那一半** —— 帧、码、记号（内核那几只手的别名与适配在 [`call`](super::call)）。
//!
//! **照实记（这一份为什么拆出来）**：帧形今天只有机器在跑，而机器只走**顺路**——边角
//! （短帧 / 长帧 / 动作码不对 / 那一串号的条数对不上 / 表外的码）一格都走不到。拆开之后这一份
//! **只认 `env` 与同层 `core`/`judge`**，宿主靶能把它逐字编进去跑判据；适配那半（内核手别名、
//! `tree()`、`ship`、两张会话失败域的映射）留在 `call.rs`。结构那一格与裁定见 `docs/frame-gate.md`。
//!
