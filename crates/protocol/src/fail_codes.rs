...
/// supply）**共用这一个号**。各家的**失败码**按自己的失败域排（同一个概念在两家是别的号——
/// 见各族 `frame.rs` 的注），而"没失败"只有一个号。`fail_codes!` 的第二个参数就是它。
///
/// **照实记（这一格原先是六份常量）**：六份 `pub const OK: u8 = 0;` 各写一遍，值全靠自律一致；
/// 收在这里最省——**本文件每个宿主靶本来就编**（见上面那两行），故一处都不多要，各家
/// `pub use crate::fail_codes::OK;` 即可。**第七份**（`programs/src/driver/rtc/call.rs`）不动：
/// 那一位的协议住在驱动自己的目录里，要不要读这一格是另一件事。
pub const OK: u8 = 0;
