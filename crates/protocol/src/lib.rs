...
//!   持板者的，**不存预约表**）。照实记：这一句原写"三个动作"，`EVICT` 上了线之后没收。两半同住一份正文，因为板线程**就住在编排域里**
...
//! - [`principal`] = **策略身份**：**名册**（TID → 此刻代表的 PrincipalId）与**谱系**
...
//!   核心、载体、服务三层都在（`prog-operator` 一个域 + `service.rs` 装配里那一格）。
//!   与 [`system::board`] 并存——那是**另一件事**（公示板 + 待客台账），不是它的旧版。
//!
//!   **"谁能动这一格"分两条轴**（见 `docs/operator-rule.md`）：**用**那一轴在门禁那一问里
