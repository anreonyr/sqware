...
//! （门闩 `HolePie`、一次往返 `Port`），而机制在 `runtime`。这一条边就是
...
pub mod uart;
// `startup` 是**启动期**那条协议（父域 ↔ 子域：靠泊 + 两条报文）——同样不进上面那张表：
// 那四个是运行期的系统语义，它是"域怎么上线"。它住 `protocol` 而不是 `runtime::core`，
// 理由与 `dispatch::control` 的 `Refer` / `Referred` 逐字相同：它就是一条协议。
pub mod startup;
// `session` 不是一个协议，是**五家共用的那一件**（判活 + 收场）：console 有会话表，
// 其余四家是一问一答，但"对面还在吗"与"这条会话怎么收场"五家都要，而它们今天各自缺
// 一半。住这里与 `startup` 同一条理由——上面那张表说的是"某一条协议的私事"。
pub mod session;
