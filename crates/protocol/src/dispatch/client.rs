...
//!
//! 本模块用 `runtime` 的机制：门闩原语（`runtime::env::mail`）与回信通道
//! （[`runtime::core::channel::Channel`]）。协议层**不碰** `env::ecall`。
...
use super::wire::{MSG_LEN, Name, REPLY_AT, Reply, Request};
...
impl Service {
    ...
    /// 入口门闩的 token（诊断 / **交给别的协议当入口**用）。
    ///
    /// 控制台的线对侧（`protocol::console::client::Console`）要的就是这枚句柄本身，
    /// 而不是"服务调用"这套带通道的包装——故这里把它交出来，调用方可按需选用。
    pub fn entry_token(&self) -> PieToken {
        PieToken::new(self.entry.token())
    }
