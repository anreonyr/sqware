...
//! 进不了「约」；而它已经在真机上量错过两次（第一版按号记 ⇒ 整道判据被跳过，
...
impl<P: Copy + PartialEq, C: Copy> Ledger<P, C> {
    ...
    ///
    /// **落树那一手为什么是注入的**（而不是收一个 `&mut Operator`）：这本账**不碰树**——收了它
    /// 就得认识树的类型。注入之后调用方既能用 `|| Ok(ID)` 单独推理这本账，也让"要位失败 ⇒ 落树
    /// 那一手一次都没被叫"这件事**可被量**（原先量它的那一台宿主靶已删）。
    ///
