...
impl<P: Copy + PartialEq, C: Copy> Ledger<P, C> {
    ...
    /// **落一枚私名**：要位 → 落树 → 记账。**三步只有这一条路。**
    ///
    /// ```text
    ///   要位（try_reserve）      失败 ⇒ 整问失败，落树那一手**一次都没被叫**
    ///   落树（plant，调用方给的） 号只有这一步答得出
    ...
    /// "先要位才写"。它挡的是**写账先于要位**那一条边——而那一条边本来就走不到（`Line` 要
    /// `id`，而 `id` 只有落树那一手答得出）；真正危险的那一条边（**落树先于要位**）它挡不住：
    /// 把调用点那三步对调成 `落树 → grow → write`，编译器一声不响，后果正是上面那条 fail-open。
    /// 收成一条动词之后，"漏掉第二步"**不可表达**——不是靠令牌提醒，是**没有第二步可漏**。
    ///
    /// **落树那一手为什么是注入的**（而不是收一个 `&mut Operator`）：这本账的宿主靶
    /// （`protocol-case` 的 `judge`）**明文"不测树本身"**（见那头注），收 `&mut Operator` 就得
    /// 让那一台现造一棵树。注入之后那一台既能用 `|| Ok(ID)` 单独推理这本账，又能**当场量出**
    /// "要位失败 ⇒ 落树那一手一次都没被叫"——这一格从前量不到。
    ///
    /// **不判"这一格归谁"**：那一问（[`Ledger::claimable`]）留在调用方、在叫它之前问。理由：
    /// 判它要"那一号还是一格 `Tile` 吗"这个事实，而那个事实今天由调用方按同一个口子注入；
    /// 次序反了的后果**不是 fail-open**（那一行照样记着主人是谁 ⇒ 客人重试即自愈）
    /// ⇒ 不值得为它再开一个名字。
    ///
    /// # Errors
    ///
    /// 备不下那一行 ⇒ [`Fail::Full`]（此时落树那一手**一次都没被叫**）；落树那一手失败 ⇒
    /// 原样转出。
    pub fn land(
        &mut self,
        at: Where,
    ...
    ///
    /// **契约：只有 [`Ledger::land`] 叫它**（它是私有的），而那条路已经先要过位 ⇒ 它不失败。
    ///
    ...
    /// `claimable`："这一格什么规矩、归谁改"）。**它是靶的读数面**：靶是**另一个 crate**，
    /// 够不着私有项，故这一格非 `pub` 不可（`#[path]` 那一版也一样——它是从靶的**根**模块叫下
    /// 来的，跨不过模块私有）；而"账上几行"也没有别的门问得出来（另一条读 `rule` 答的是规矩，
    /// 不是行数）——故留着它，并把读者写在这里。
