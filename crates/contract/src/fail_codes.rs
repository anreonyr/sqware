//! `fail_codes!` —— **码表宏自己一份源**（协议各家同读这一份）。
//!
//! **照实记（它为什么住一个文件）**：这个宏原先写在 `lib.rs` 里（`#[macro_export]`）。
//! 挪出来之后**调用点一行都不用改**，靠的是**两样都要**：`#[macro_use]`（把宏带进本 crate
//! 后面那些模块的作用域）与 `#[macro_export]`（保住"出 crate"那一份）。
//!
//! **照实记（原来那条理由已随宿主靶撤掉）**：搬它出来的直接起因，是那台编外宿主靶要读同一份
//! 而靶**不依赖 `protocol`**（它拖 `runtime`，宿主编译器编不出那两处 riscv 内联汇编）。靶已删
//! （用户裁定"protocol-case 没必要"），那条理由随之撤；**本文件照旧独立**——上面那两样
//! （`#[macro_use]` 的正文先后 + `#[macro_export]`）今天一样成立。
//!
...
/// 收在这里最省——**本 crate 的每一份用家本来就看得见它**（见上面那一份照实记），故一处都不多要。crate 内各家
