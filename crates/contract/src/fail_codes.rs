...
//!
//! **照实记（宿主靶那一侧的用法后来变了）**：靶起初照抄那两行（`#[macro_use] #[path] mod
//! fail_codes;` 写在帧模块之前，因为那时源码是被逐字搬进靶的、`crate` 指靶）；靶改**真依赖**
//! 之后它不再自己读这一份——`frame.rs` 在 `contract` 里编，`crate::fail_codes!` 落在**这里**
//! 的根上，靶那一侧一处都不用写。
//!
...
//!
//! 结构那一格：本宏独立成一份源——协议与宿主靶**同读这一份**（靶那一侧如今走依赖边）。
//!
//! ---
//!
