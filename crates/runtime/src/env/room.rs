...
///
/// 失败：`Dead`(-2) 目标从未入册 / 它所属的域已不在世；`Denied`(-1) 不在血缘里
/// ——**"与发起者同域"与"目标是顶级域"都落在这一支**（前者在"严格后代"里被吃掉，
/// 后者 `sire` 恒空），不是 `-2`。**不等它回收**——要等用 [`crate::env::unit::join`]。
///
/// **今天没有调用者**：他杀是给域间政策留的位置，而政策服务还没落地（域侧一律自愿
/// 退场，级联由内核在父退场时自己跑）。留着它是为了让"能不能"这句话有个可被调用的
/// 形状——不是为了让谁现在就能杀别人。
