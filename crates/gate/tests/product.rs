//! 产品镜像那一门（`SQWARE_ROOT=product`）——**真正要发出去的那一景**（用户裁定"7 台"）。
//!
//! # 判据（两段：装了什么 · 起不起得来）
//!
//!   1) **装了什么**（宿主侧，读回那颗 `initrd.img`）：**整表**恰好 9 条 = 两个域（`root` /
//!      `system`）+ 7 台（6 服务 + `echo`），且引导镜像落在 `root` 上——`product` 与验收镜像
//!      **共用同一个引导域**，故"景名 = 引导镜像名"那条旧巧合在这里断了
//!      （见 `env::assembly::ENTRY` 的照实记）。
//!   2) **起不起得来**（QEMU）：自己退场 + 停机行 + 六沓用例全绿 + **恰好 7 条 `system: gone`**
//!      （编排域自己的账——起一条记一条，故它是"这一景真起了哪 7 台"的机器判词）
//!      + 探针与常客的读数**一条都不在**。
//!
...
//! # 照实记（`board:` 那一行**不能锚行尾**）
//!
//! 实测到过 `board: swept n=1 occupied=4system: gone principal …`——两条读数**胶在一行**上
//! （现场 `target/product-run.log`）。调试面是按行打的（`debug::put` 一次一句），而写出落到
//! SBI 那一侧**不是一次原子写**，于是两个线程的行能交错。故这里判 `board:` 只判**子串**。
//!
//! **照实记（同一个坑在 `soak` 那边是假红的来源）**：`soak` 的 `board:` 形状锚了 `$`
//! （`^board: (bye …|swept …)$`），胶起来的那一行两边都不匹配 ⇒ `Tier::Auto` 逐行判 ⇒ 报
//! "`board:` 每一行都要被判"。旧现场里量到 7 例（`soak-*` / `framework-*` / `console-*` 各一份），
//! 与"一行读数该不该在"无关。
...
/// 这一景该装的那 9 条（**镜像层**：装载次序）。次序是硬事实（`ROOT_OFFSET` 按位次算），
/// 故这里是**整表相等**，不是"含这 9 个"。
const PACKED: &[&str] = &[
    "root", "echo", "router", "uart", "rtc", "principal", "coalition", "operator", "system",
];

/// 编排域起的那 7 台（**单子层**：按 `order` 排出来的次序）——`echo` 在最后（等它退场才收场）。
const PLAN: &[&str] = &["operator", "principal", "coalition", "router", "uart", "rtc", "echo"];
...
fn product() {
    ...
    // 起的那 7 台：编排域自己的账（起一条记一条）——**整表相等**：多一台少一台都点名。
    ...
    println!("product: 9 条镜像 · 7 台在起 · 六沓用例全绿 · 停机行在");
}
