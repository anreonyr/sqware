...
const RAW: &[Raw] = &[
    ...
    Raw { name: "供·已知坐标也去查表（`settle` 两格不分）", path: "crates/env/src/wire/supply.rs", from: "            At::Known(key) => key,", to: "            At::Known(_key) => of(\"\")?,", at: Where::Host, hope: Hope::Red },
    ...
    Raw { name: "机器·编排域的判词不落（该有 `system: done`）", path: "programs/src/supervisor/system/main.rs", from: "    Ok(programs::Report::note(env::EXIT_OK, \"system: done\"))", to: "    service::die(service::E_OK, \"system: farewell\")", at: Where::Soak, hope: Hope::Red },
