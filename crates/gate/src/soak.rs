...
pub const READINGS: &[Reading] = &[
    ...
    Reading { prefix: "rtc", tier: Tier::Narrative { shapes: &["^(rtc: got [0-9]+|rtc: time [0-9]+ -> [0-9]+|rtc: asked now=[0-9]+|rtc: legs t1=[0-9]+ t2=[0-9]+ unpack=[0-9]+ find=[0-9]+ dev=[0-9]+ slot=[0-9]+ dwrite=[0-9]+ say=[0-9]+ push=[0-9]+ rel=[0-9]+|rtc: refused=[0-9]+ at=[0-9]+ now=[0-9]+ late_ns=[0-9]+|rtc: tree part=[0-9]+ dir=[0-9]+ land=[0-9]+ find=[0-9]+ got=(true|false) entry=[0-9]+ plate=[0-9]+ pname=[^ ]+)$"] } },
