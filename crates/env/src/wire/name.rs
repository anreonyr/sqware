...
//! 身份只留号），**认设备那一刀又收了一处**——配对块里那一格式**设备 basename**，今天换成了
//! 坐标（[`crate::wire::key`]），内核连"读出来打一行"都当场丢掉。故这个上限今天**只钉用户态
//! 协议**；内核侧只剩一处**解码面**的引用：孔记号（用户给的**不透明匹配键**）在 envcall 入口
//! 按 `Name` 的口径校验过——那不是内核的名字，是它替用户挡的非法字节。
