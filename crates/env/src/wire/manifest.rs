...
//! 处是内核的打包表**（`kernel/build.rs 的清单（`PRODUCTS` / `PROBES` / `RIGS`，按场景选表）`），域只读取并原样转交 `Build`。
//!
...
/// 清单条数上限。
///
/// 32 = 单张镜像最多 **21 台** + 11 格余量。
///
/// **照实记（这一格连着撞过三次）**：它原先写 20，理由那句是"今天的 17 个程序"——17 是写那句
/// 话时的数，后来加到 20 个时没人回头看它，于是"留三格余量"从那一天起就是假的（20 正好卡满）。
/// 那一刀（`/device/rtc` 那面服务带来第二十一个程序）抬到 24；再一刀（结盟服务 + 它的探针）
/// 抬到 28；又一边（规矩那一格的两台证客）撞满 ⇒ 抬到 32——**还是"+3"**。三次症状都一样：
/// `pack` 返 `None`，内核 build.rs 报"清单越界"。
///
/// **这一刀（用户裁定"测试和程序分开"）之后，压力的成因没了**：清单不再是并集那 31 条——
/// `kernel/build.rs` 按场景选表（`PRODUCTS` / `PROBES` / `RIGS` + `bins_for`），单张镜像最多
/// 21 台（验收：15 产品 + 6 探针），台子那几张只有 1~3 台。故 32 今天有 11 格余量。
///
/// **要真收口**：让 `pack` 的数与 `bins_for(场景).len()` 同源——今天一个是编译期常量、
/// 一个是打包表，靠人抬。
