...
/// 清单条数上限。
///
/// 32 = 今天的 29 个程序（`kernel/build.rs::INITRD_BINS`）留三格余量：加一个程序不必
...
/// 那一刀（`/device/rtc` 那面服务带来第二十一个程序）把它按同一个意图抬到 24。
/// 再一刀（**结盟服务 + 它的探针**带来第二十四个与第二十五个程序）按**同一个意图**抬到 28
/// ——同一条照实记已经记过一次，故那次把数写准了：`INITRD_BINS` 25 条，28 = 25 + 3。
///
/// 这一刀（**规矩那一格的两台证客**带来第二十八与第二十九个程序）又撞上了同一格：28 已经
/// 卡满，`MAX_PROGRAMS` 不够，内层 cargo 之后 `pack` 直接返 `None`（真机症状是内核
/// build.rs 报"清单越界"）。抬到 32 ——**还是"+3"**。这一格连着撞三次，症状都一样：
/// **上限跟着程序数走，而程序数每次都是"顺手加一台"**；哪天真要收口，该让 `pack` 的数
/// 与 `INITRD_BINS.len()` 同源（今天一个是编译期常量、一个是打包表，靠人抬）。
