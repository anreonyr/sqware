...
// 两位消费者：
//   - **建域权**（`UnitCall::Build`）：按 token 在**调用方自己表里**找一枚活着的 Nole；
//   - **门铃**（`Bell`，`crates/runtime/src/core/bell.rs`）：把一枚 Nole 当"内核一件
//     事实的出口"用——等它响、应它、由内核响它（`docs/bell.md`）。
//
// 第二位消费者带来的是**听者面**（`id` / `life` / 一位 `ring`）：门铃有听者，故有
// 等待键；响要能被"响在没人听的那一刻"记住，故有一位。它**不带来第二种类型**——
// 门铃仍是一枚 Nole，"怎么用"封装在 runtime 层。
//
...
//
// 边界（定义式）：Nole 只承载**无载荷**的存在权。带状态的许可（配额"最多 N 个"、
...
pub struct NoleMeta {
    ...
    /// （响着 ⇒ 本 hart 的 SEIE 关着），见 `docs/bell.md` §2.1。
    ring: SpinLock<bool>,
...
/// 闸门**的信号（`trap.rs` 的外部中断分支），也是"多 hart 同响合成一位"的落点。
///
