...
//
// 载体是 `AnyPie::Nole`（`work::mail::nole`）：无数据面的权柄载体。为什么必须是
...
// 的 Nole"——**按 token 在调用方表里找**，故 token 不自证，"借来的 token"不是绕过面。
//
// **第二位消费者 ＝ 门铃**（`nole::ring`/`hush`，`docs/bell.md`）：一枚 Nole 当"内核
// 一件事实的出口"用。故"Nole 没有别的用途可被误用"这句话不再成立，取而代之的是
// **门二（S 态）**：门一答"你有没有存在权"，门二答"这份存在够不够伸到沙箱外"。
// 判据本身（下面这一行）不变。
...
/// 调用方是否持有建域权：**它自己表里**有一枚活着且是 `Nole` 的门闩。
///
...
pub(crate) fn holds_build_right(task: &Task, token: usize) -> bool {
    ...
    pies.iter()
        ...
        .is_some_and(|p| matches!(p, AnyPie::Nole(_)) && p.alive())
}
