...
/// 代码"；而被他杀的受害者是在**别的核**上被 IPI 唤起、自己在 `trap/mod.rs` 里自退的
...
/// 直到关机级联才被收掉（实测：`Doom` 答 `Ok`、目标 300 ms 内没自退，见 `crates/gate/src/soak.rs`（已删））。
///
...
/// 谁收（会话的收尾由会话的主人负责，见 `programs/src/system/main.rs` 的 `board::shut`）。
