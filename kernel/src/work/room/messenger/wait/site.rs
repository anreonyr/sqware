...
pub(in super::super) struct Site {
    ...
    /// 落在登记侧（`forward` 返 `Result`，`attach` 有失败域）而不是唤醒侧。
    ///
...
///   - 撤登记侧（`unforward`，被 `detach` 与 `Tole` 的 `Drop`/封印调用）：**`Drop` 不能失败**
///     ⇒ 摘一格也不能分配。
///
/// 固定容量让两条路都退化成"改几格数组"，于是容量账全部落在**登记侧**：`forward` 返
/// `Result`，`attach` 有失败域，满了答 `OoM` 并**把刚挂的那一格退回**（不留"挂着却叫不醒"
