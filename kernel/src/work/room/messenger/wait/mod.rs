...
fn block(key: WakeKey, life: Weak<Life>, dur: Duration) -> Result<Handoff<()>, GateError> {
    ...
    if let Some(at) = at {
        ...
        //
        // 照实记（量过 · **已按对齐环境重取**）：正因为 `redeem` 全局，**任何一颗空闲核**都会
        // 按 `due()` 武装并兑现全部到点 ⇒ 树内 workload（soak 全员 1ms 轮询睡眠、rig 只有一枚
        // churn）里总有核空闲，这条债**量不出来**（release soak 16k 样本 A/B 无差别）。要量它
        // 得把"空闲核"从机器上删掉：单核 + 占核者 + 一枚**稀疏**打点者
        // （`QEMU_SMP=1 scripts/load.sh 1 --release`）。实测（**icount 关**、release、n=81，
        // 同一台子只差那三处武装式子）：**修复前 `late_avg=97 ms / late_max=99 ms`；修复后
        // `0 / 0 ms`（`late_max_tick` 逐轮 2732~5850 抖动，即 0.27~0.59 ms）**；`traps` 两边
        // 都是 **643** ⇒ 没多花中断。
        // 多核对照档（`QEMU_SMP=4`：债被空闲核盖住）与逐条读数见 `programs/.../stress/load.rs`。
        //
        // 也是那次实测发现：**这一句在本核里是冗余的**——停下一枚任务后，那颗核必然会再武装
        // 一次（`seat` 下一枚 → `hart.rs`，或进空闲 → `fetch.rs`），所以只把这一句关掉，忙机台
        // 上的迟到仍是 0（重取：6 打点者档**毫秒那几格确实不变**——`late_n=281 late_avg=0
        // late_max=0 ms`；只有亚毫秒那格不同：关掉 744 µs / 留着 396 µs）。留着是**保险带**：
        // 让不变量在"登记那一刻"就成立，而不是依赖"这颗核稍后一定会再武装一次"；删掉则少一次
        // SBI ecall、少一处改动——**留/删待裁**。
        //
