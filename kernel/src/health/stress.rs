...
pub(super) fn accept() {
    ...
    // 判据之前先还掉**本用例自己的**宿主缓冲：它们都**未标注** ⇒ 活着时正好记在
    // `plain` 那一桶（实测：`drained`（上万项、上百 KiB、走 frame 大块路径）不 drop
    // 就报 `plain: 3 → 4`；`held`/`fheld` 是块侧（≤ 2 KiB）各一笔，不 drop 就报
    // `plain: 20 → 22`）。那是用例的账，不是分配器的账 —— 判据只该看分配器的。
    drop(drained);
    drop(held);
    drop(fheld);
