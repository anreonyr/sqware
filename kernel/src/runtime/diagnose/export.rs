...
//!
//! 两份 wire 并存（裁决 D2-b：保留事件实时镜像）：
//!   · 事件流：运行时逐条实时（[`push`] 一条 JSON 行，`trace::host_note` 经此）；
//!   · 报告快照：[`export`] 整档报告一次 `serde_json::to_vec` 追加一行（seal
//!     后由发刊方调用）。
//! 消费端按行解析：事件行 = 小对象（含 kind），报告行 = 大对象（含 paras）。
//!
//! 序列化全交 serde_json（零手写 JSON）；文件 crate 经 HOST 锁 try_lock 串行
//! （跨核事件写），失败闩停用——崩溃核写报告时其它核已 hunker，锁不会死等。
...
/// 导出文件名（qemu 进程 CWD；runner 结束后归档）。
...
/// 量级内必完成；超限即放弃——防 panic 现场对已 hunker 的持锁核死等。
...
/// 推一条 JSON 行（事件流实时镜像；`json` 须为合法 JSON 文本）。
pub fn push(json: &[u8]) {
    with_host(|f| {
