...
///
/// 语义改名，不是新数：它就是原先"抢占量子"里写死的那个 `100ms`（`trap/mod.rs`、
/// `hart.rs`、`stack.rs` 三处各写一份）。改名是因为它管的不止"跑多久换人"：
/// `tock` 只入堆、不碰 `stimecmp` ⇒ 到点是否按时兑现全看**武装**那一式（[`beat_until`]
/// 把武装点收到 `min(上限, 最近活到点)`）；漏掉的那一拍由失明的核最迟在 `BLIND_MS` 后
/// 醒来重算 ⇒ 自愈。这也是「不变量不会永久破」的来源：破口寿命 ≤ `BLIND_MS`。
