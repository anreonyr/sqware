// 时钟模块（clock）— 全内核时间源 + 单位换算
//
// 职责：只回答「现在几点了 / 过了多久」——时间读数 + Duration 换算 + tick 基准。
// 与同级的 timer 模块分工：clock 不含任何 deadline 登记 / 武装语义（那是 timer
// 的事）；timer 依赖 clock（now / 换算 / Instant），clock 不反向依赖 timer。
//
...
// ── 时间换算（模块内部；pub(crate) 供 timer/trap 使用）──
