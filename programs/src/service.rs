...
pub struct Program {
    ...
    /// 准入由门外那一问判，见 `protocol::system::operator::{judge, gate}`）。
    pub operator: bool,
...
pub fn assemble<'a>(
    ...
    for (role, p) in roster {
        ...
        if p.holds_tree {
            tree = Some(task);
            otip = None;
            operator::host_of(task, Wait::AtMost(READY_MS), &mut otip).map_err(|why| {
                step(p, why);
                p.died
            })?;
        }
...
pub fn wait_last(table: &mut Table, name: Name) {
    while let Ok(false) = service::watch(table, name, Wait::Forever) {}
}
