...
//! **读数**：`user/lodger`（房客）每次冷启动都占住 11 号线、一句话不说就走 ⇒
//! `router: line 11 = rtc@101000` 与 `router: vacate line=11`（两道门都当固定读数）。
//!
...
/// 故"收线"不靠板、也不靠一拍。
///
/// **这一跳有读数了**：`user/lodger`（房客）每次冷启动都占住 11 号线、然后一句话不说就走
/// ⇒ 本域被叫醒、`alive` 答不出 ⇒ `router: vacate line=11`（两道门的固定读数，见
