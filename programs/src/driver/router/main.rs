...
//! router — **线路由者（中断面域）**：外部中断的收与结（S 态 supervisor 域，**一枚线程**）。
//!
...
//! **那一拍不再兼排空的替身**（旧 `IRQ_WAIT_MS` 是"客户端说不出排空"时代的替身）；今天还剩
//! 一拍 [`IRQ_POLL_MS`]，只为另一件事——**唤醒丢了时的保险**（见 `main` 里那一注）。
//!
...
/// 唤醒丢了时的保险那一拍（毫秒）。**不是排空的替身**（那是事件），见 `main` 里那一注。
const IRQ_POLL_MS: usize = 20;
...
extern "C" fn main() -> ! {
    ...
    // 另有一拍 [`IRQ_POLL_MS`]，那是唤醒丢了时的保险，不是这三样里任何一样的替身。
    ...
    loop {
        ...
        //
        // **这一拍不是"排空的替身"**（排空已经是事件，见下），它是**唤醒丢了时的保险**：
        // 实测偶尔有一次组等待没被叫醒——PLIC 的 `pending` 置着、闸门开着，而本域睡着
        // （字节留在设备里、随后一枚中断都不来）。**有界等兜住它**：醒来 `claim` + `hush`
        // 一次就自愈（`hush` 会重开闸门，pending 还在 ⇒ 立刻再报）。
        // **这个数没有依据**（照实记）：取 20 ms，与旧 `IRQ_WAIT_MS` 同量级——那时它兼着
        // 排空的替身，今天只剩这一件事。
        match tole.await_(IRQ_POLL_MS) {
            Ok(Some(_)) => {}
            // 期限到（或挂起过）：照样往下走一遍——`claim` 领到空就什么也不做。
            Ok(None) => {}
