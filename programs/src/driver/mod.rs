...
//! `kernel/build.rs::INITRD_BINS` 声明自己是哪一档——今天三台都是 **`User`**：这一格是量出来
...
//!
//! **三块都挂上了**：`router` 是 `/device/router`（`part=0` = 那块 Pane 是它建的），`uart` 是
//! `/device/uart`、`rtc` 是 `/device/rtc`（`part=2` = 已经在了）——后两块各挂**一扇服务门**。
//!
//! # 服务面（两台都有客人了）
//!
