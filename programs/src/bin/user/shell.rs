...
///
/// # 写缓冲：按行合并，**一次 flush 落成一条消息**
///
/// `write` 先攒进 [`Terminal::buf`]，**遇换行或满 [`Terminal::CAP`] 才发**。两个数为什么
/// 是同一个（`Terminal::CAP` ＝ `protocol::console::LINE` ＝ 128）：一段字节若超过协议
/// 那一条上界，客户端就得把它切成**两条**消息，而两条之间可以插进**别的写者**的字节
/// ——实测一句 27 字节的报到被劈成 `…reconnect` + 别人的一句 + `ed`，门里的判据因此常年
/// 发飘。取同一个数之后，一次 flush 永远落成一条消息。
///
/// 读行前强制 flush——次序不能颠倒，否则提示符会晚于读行落屏。
///
...
impl Terminal {
    /// 一次 flush 的上界：**必须等于**协议那条消息的上界（`protocol::console::LINE`），
    /// 否则一条消息会被切成两条、让别的写者插进中间（见本类型的头注）。
    const CAP: usize = console::LINE;
}
...
fn reconnect() -> bool {
    ...
    for _ in 0..CONSOLE_RETRY {
        // 报到在 [`with_session`] 里（会话建成就报），这里只问成没成。
...
///
/// 读数 `ok` 的牙在**会话表不随重开增长**上：会话的归属是推者，重开一次是**换掉那一格**，
/// 不是新占一格。**反向实测**：把这半条去掉（重开各占一格，其余不动）⇒ `ok` 从 12 变
/// **7**（表宽 8 被 12 次开吃穿）；旧形状（开一次占两格）第 5 次就没。
///
...
/// [`console_entry`]）——理由：既有配给机制（`Refer{who}` → dir 控制线程 `ship`
...
fn exec(cmd: &str, args: &[String], term: &Term) -> bool {
    match cmd {
        ...
        "churn" => {
            ...
            // 水位读数（`idle Δheld/Δwalk`）随帧池探针一起撤：`churn` 现在只报轮数、
            // 形状与耗时 —— 生灭本身照样压到位，读数归内核的关机判据。
            for pass in 1..=rest {
                let _ = pass;
                churn(term, n, d, f);
            }
