...
//!
//! **两张表合一**：注册表即预约表（见 `protocol::dispatch::server`）。控制线程要往里
...
//!
//! 注册表逻辑在 `protocol::dispatch::server`；协议规范见 `docs/dispatch.md`。
...
extern "C" fn main() -> ! {
    ...
    if Quay::new(at_parent).push(&up).is_err() {
        runtime::env::room::exit_with(11);
    }

    // 6. 服务循环。
    let mut msg = [0u8; CALL];
    loop {
        // 身份 = **内核盖章的发送者**（`Pull` 一并交回），不信任报文里的任何字段。
        // 长度也一并交回：载荷按上界给，尾部有多余的零，而帧长只有 `Pull` 知道。
        let Ok((len, from)) = entry.pull_from(&mut msg) else {
            continue;
        };
        let caller = from.get();
