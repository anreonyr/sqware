...
//!
//! ```text
//!   ① 台主造：共享组（Unseal{shared:true}）+ 成员孔 + **两枚**回报孔（一人一枚）
//!   ② 两个子域各一枚线程，各 accord 一份（组 + 成员 + 自己那枚回报孔）→ hatch
//!   ③ 每个等待者：hang(成员, Pull) → 报 "H" → Await(MAX)     ← 两人同时挂同一只组键
//!   ④ 台主收齐两份 "H"（**收到才投信**：判据的前提是两人都在等）
//!   ⑤ 对照：独占组在同一位置上的**第二次** accord 必须被拒（源枚已 `Caged`）
//!   ⑥ 稳压 200 ms → 成员孔投一字节 → 两个等待者都该被放行
//!   ⑦ 两人各自 `peek`（**非破坏性**，一个字节都不取）后回报 "T"
//!   ⑧ 台主自己取那条消息：一次成功、再一次 `Busy` ⇒ "交付只归一人"
//!   ⑨ 台主汇总：hung=2 woke=2 deliver=true control=true ⇒ PASS
//! ```
//!
...
//!   `ONLY` 才是"移交"：第一次成立、第二次因源枚已 `Caged` 被拒。**那次拒付本身就是
...
extern "C" fn main() -> ! {
    ...
    //    `Caged`）。放在这里是因为此刻两个等待者都停在 `Await` 上、表不再变。
