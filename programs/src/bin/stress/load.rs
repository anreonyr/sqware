...
//! **三处武装点的式子**（`trap.rs` / `hart.rs` / `stack.rs`：`beat(写死 100 ms)` →
//! `beat_until(min(上限, 最近到点))`）与 `wait::block` 里**新增**的那一次"登记后当场重武装"：
//!
...
//! ④ 只把 `wait::block` 那次"登记后当场重武装"关掉（6 打点者档）：**毫秒那几格确实不变**
//!    （`late_n=281 late_avg=0 late_max=0 ms`），只有亚毫秒那格不同（关掉 744 µs / 留着
//!    396 µs）⇒ 承重件是那三处武装点的式子，这一句是**保险带**（见 `wait::block` 的照实记）。
