...
//!
//! 三条结论：
//! 1. **卡点是"源核不 yield"**（这是本轮最重要的发现）：被唤醒的任务由 `rise` 推进**唤醒那颗核**
//!    （= 台主）的就绪队列，然后 `kick()` 只叫醒**一枚**等待核去 `steal`；而 `kick` 的兜底写着
...
//! ③ 让出一拍之后，`d` 扫的不再是"相对它上台那一刻"的偏移（那一拍把时序交给了调度器）
//! ——**要恢复精确时序，得先修内核那条兜底**（让唤醒直接落到被叫醒那颗核的队列上，而不是
//! 等源核 yield），那是下一件。
//!
...
/// 台主在 push 之后**先让出一拍**再空转（**默认开**——关掉它这台子就够不着要量那一格）。
///
/// 为什么非让不可：被唤醒的任务由 `rise` 推进**唤醒那颗核**（= 台主）的就绪队列，`kick()` 只叫醒
/// 一枚等待核去偷，而它的兜底是"源核下次 yield 自取"——台主是 S 态域任务、空转不吃陷阱
/// ⇒ **永不 yield**（见 `conductor::kick` 的照实记）。实测：关掉它 `starved=318/328`、
/// `nudged=3`、`lost=0`；开着它 `starved=0`、`nudged=146`、**`lost=2/328`**。
///
/// 代价照实记：这一拍把时序交给了调度器 ⇒ `d` 不再是"相对它上台那一刻"的精确偏移。
/// 要两样都要（精确时序 + 它真在台上），得先修内核那条兜底。
const YIELD_AFTER_PUSH: bool = true;
