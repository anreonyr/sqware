...
env::frame! {
    ...
    /// 才显形（正是 [`Eyes`] 那一段照实记里同一个毛病的第二次）。先把两个常量并成一个，这一刀再并
    /// 成上面这张字段表：`16` 这个数从此**一处都不写**，两侧读的是同一个 `LEN`；后 8 字节怎么翻也
    /// 不再由装配侧按 `Eyes::of_wire` 现算、由收侧按 `from_le_bytes` 现翻——那是 [`Eyes`] 自己的
    /// `Field`（读不懂 ⇒ 整帧读不懂，收侧照旧报一句）。
    ///
