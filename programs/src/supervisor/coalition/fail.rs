//! coalition 这一域的**错误类型**——`serve()` 的返回类型，`?` 一路把它带出来。
//!
...
impl Fail {
    ...
    const fn text(self) -> &'static str {
        match self {
            Fail::Sire => "coalition: no sire",
            Fail::Board => "coalition: board",
            Fail::Tree => "coalition: tree",
            Fail::Desk => "coalition: desk",
            Fail::Face => "coalition: no identity plate",
        }
    }
}
