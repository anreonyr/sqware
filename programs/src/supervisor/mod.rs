...
//! 外**都是 `Supervisor`。**照实记**：[`principal`]（身份服务）是 **U 态**——它不持有、不授予、
//! 不解释任何 Pie，只读写自己那两张表，故不需要 S 态；它住这里是因为目录按**角色**分
//! （同名服务的判定与接口住 `crates/protocol/src/principal/`），不是因为特权级。
//!
//! - **实现**：[`supply`]（引导域那圈发货
...
//!   `operator/main.rs`、`principal/main.rs`、`system/main.rs`。
