...
//!
//! **那一轮的读数是 168 次试验/轮**（那一版粗扫的档距与今天不同：今天同一台子是 **328 次/轮**
//! ——`PER_DELAY = 8` × 41 档，见下面 rig A 那张表）：`rig: total n=168 now=166 waited=2
//! late=0 lost=0`。也就是说**绝大多数轮里受害者根本没上过台**——台主（造它的那颗核）不阻塞时，
//! 新放行的受害者在**本核**队列里等着，别的核拿不到它（今天核间**没有"偷"这条路**：`steal`
//! 已整条删掉，见 `scheduler/core/fetch.rs`；这几行是它还在时量到的数）。kernel 侧计数：
//! `cull_v=7 nudge=1` ⇒ 8 次里 7 次是"在容器里被杀"、1 次是"在台上被杀"。故这台子当时量到
//! 的是**快路径**：
//!
//! | 被杀时它在哪 | 结果 |
//! |---|---|
...
//! |---|---|---|---|---|---|---|---|
//! | 台面 20 millis，扫 d ∈ [0, 20 millis] | 328 | 324 | 4 | 0 | 0 | 0/324/0/**4** | 977/31/443 |
//! | 台面 20 millis，扫 d ∈ [0, 200 millis]（步 5 millis） | 328 | 328 | 0 | 0 | 0 | 0/328/0/**0** | — |
//! | 台面 100 s，d = 1 / 20 / 100 millis（各 32） | 96 | 95 | 1 | 0 | 0 | — | — |
...
//!    不是位置：20 millis 台面下 324/328 是 `now`，而分支计数说真正"在台上被杀"只有 4 次。故本台子
...
//! 门铃确认重试、整字广播、空闲核 1 millis 有界兜底拍。**这些补救全部白费**——真因是
//! **台子与验收门跑在两个环境里**：
//!
//! - `scripts/boot.nu` 的默认是 `-icount auto,sleep=on`：按宿主时间给 vCPU 记账、让它
//!   睡够虚拟额度 ⇒ **WFI 里的核被 IPI 叫醒要等额度**（延迟直方图众数 **1~10 millis**，
//!   尾巴到 10~100 millis）。这解释了当时看到的全部现象：投活 980 笔里 976 笔落在正睡在
...
//! | 落点核自己取走活（`own`） | 24 | **896~1142**（**该读数已随 `sched:` 行改版退休**） |
...
//! 点名正好落在受害者**离核那一瞬**（`d_us ≈ 20 millis`）时，杀令**记上了却没有落点**——
...
//! 把 20 millis 附近按 25 µs 细分（[`EDGE_SWEEP`]，默认关）之后它成了每轮 0~3 次的可测事件
...
//!
//! 另一处要小心：Doom 之后的判决窗口（300 millis + 1 s 宽限）**长于**台面，故 `late`/`lost`
...
//! **`Park{millis}` 在"有任务的核"上按拍兑现**（定时器在 trap 里固定重武装 100 millis；只有核进
//! 空闲取活时才按最近的 tock 武装，见 `scheduler/core/fetch.rs`）⇒ 域里 `sleep(1 millis)` 实际是
//! "到下一拍"，`churn` 那个"1 millis 在台上 / 1 millis 离核"的换算因此不成立（它的睡眠段其实是量子级）。
//! 现在四处武装统一为 `min(本核上限, 最近活到点)`（`trap/mod.rs` / `hart.rs` / `stack.rs` /
//! `fetch.rs`）⇒ 该换算成立，本台子的档距假设（下面的 `DELAY_MAX_US`）应当按 `sleep(1 millis)`
//! 的真实时长重取。（**照实记**：这一句原先还跟着"且登记到点的那颗核当场按同一式子重武装"
//! ——那句话**已按用户裁决删掉**，见 `messenger/wait/mod.rs` 的"已裁：删"，故不再作数。）
//!
//! # 读数
//!
...
//!
//! `lost` = **300 millis + 宽限 1 s 都没收掉**——那才是"他杀没生效"；`late` = 迟到了但收到。
...
/// 边界细扫开关（**默认关**，见 `main` 里那一段）：把 `d_us` 在 20 millis 附近按 25 µs
/// 细分再扫一遍。开着一轮 656 次试验。
///
...
///
/// 20 millis 是**扫得动**的台面：档距 500 µs ⇒ 40 档覆盖一整个"在台上"。
...
extern "C" fn main() -> ! {
    ...
    // `d_us ≈ 20 millis` 出现——粗扫一档 8 轮抓不到几个样本。把 20 millis 附近按 25 µs 细分
...
fn body(
    ...
    // 一枚新任务，自己校准等于每轮白扔 0.4 s（睡 200 millis + 忙等两格刻度）。
