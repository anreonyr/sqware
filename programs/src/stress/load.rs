...
//!   → 台主自己也每 1 millis Park 一次（让出核，别霸占）
...
//! workload 全都满足"总有核空闲"（`soak` 全员 1 millis 轮询睡眠、`rig` 只有一枚 `hang`）⇒
...
//! 当初 icount 开时**同值**（97 / 99 millis）；变的只有 `traps`（645 → 643，±2 的计数噪声）。
//!
//! # 两个已经踩过的坑（都不是明显的那一个）
//!
...
//! - **四处武装点**退回"写死 `beat(100 millis)`"：1 millis 的到点只能等**下一个量子拍**被兑现 ⇒
...
//! **四处武装点的式子**（`trap/mod.rs` / `hart.rs` / `stack.rs` / `fetch.rs`：`beat(写死 100 millis)`
...
//!
//! 四点：
//! ① **债是真的，量级正好一个失明上限**（99 millis ≈ `BLIND_MS`）——单核档两边 `late_n`/`tocks`
//!    都是 81/81，同一条命只差武装式子。
//! ② 单核档陷阱数两边一致（**643 = 643**）⇒ 到点稀疏时"到点密 ⇒ 陷阱密"这条代价并没有兑现
//!    （节拍仍由量子决定）。修后的 `late_max_tick` 逐轮抖动（四个轮次 2732~5850，即
//!    0.27~0.59 millis），表里那一格是其中一轮；量级不变。
//! ③ 多核对照档正说明"核数就是这条债的开关"：改前只剩 4 millis 的尾巴（`traps=3`），改后
//!    `late_max=0 millis`、`traps=31`——多出来的拍全花在"按最近到点武装"上（那句式子的代价面）。
//! ④ `wait::block` 那次"登记后当场重武装"（**已按裁决删掉**）：6 打点者档把它关掉，
//!    **毫秒那几格确实不变**（`late_n=281 late_avg=0 late_max=0 millis`），只有亚毫秒那格不同
...
///
/// 到点密的台子量不出这条债：多枚 1 millis 打点者会让"最近到点"永远存在，于是**光靠陷阱路径
/// 那一句 `beat_until` 就已经自洽**（实测 6 枚打点者档：毫秒那几格 `0/0`，亚毫秒那格只差
/// 744 µs / 396 µs——见下④）。要让"武装式子"这件事**可判**，到点必须**稀疏**：打点者两次
/// 登记之间堆是空的 ⇒ 上一次武装只能按失明上限（100 millis）⇒ 式子不对就必然晚到量子量级。
const PARKERS: usize = 1;

/// 台主自己睡多少次（每次 1 millis）。台主也是打点者之一（头注坑 2），故它的到点同样被测。
///
...
extern "C" fn main() -> ! {
    ...
    // 台主自己：每 1 millis 让出一次核（**不许纯空转**，见头注坑 2）。
