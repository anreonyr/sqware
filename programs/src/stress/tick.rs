//! tick — 压测台**各程序共用**的**测时与空转**（见 [`crate::stress`]：这些程序互不依赖，
//! 共享的只有这一份"怎么量时间"）。
//!
//! # 域能读到的钟只有一格
//!
//! `Chrono::Ticks` 数是**内核的定时器中断**（`timer::tick()`），不是时基——实测它的间隔就是
//! 调度量子 100 millis。故本模块的"刻度"= 那个计数，`ms_per_tick` 由 `sleep` 量出来，
//! `iters_per_tick` 由空转一段数出来（**等一次完整的刻度间隔**，不是半格）。
//!
//! 这些程序量的是同一把尺：受害者据此把"在台上"做成 1 millis，台主据此把"点名时刻"扫到微秒。
...
///
/// 刻度 = 定时器中断计数：先量它一格有多久（用 `sleep` 当基准，**不假设**它是 100 millis），
/// 再空转着数满**一整格**（等计数从 `a` 走到 `a+1`），于是 `iters_per_tick` 是真的整格。
pub fn calibrate() -> (usize, usize) {
    // 一格多少毫秒：睡 200 millis，看计数动了几格。
    let t0 = now();
