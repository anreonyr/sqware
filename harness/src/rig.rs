...
//! ```
//!
//! `crates/gate/tests/stress.rs`（已删）的 profile 默认因此是 `--release`（想复现 debug 那一挂：
//! `crates/gate/tests/stress.rs 1 --debug`（已删））。
//!
//! # 照实记（装好台子当天量到的）
//!
...
//! （`QEMU_ICOUNT=`，见 `crates/gate/tests/stress.rs`（已删））之后，同一台子的读数是另一套（见本文件后半
...
//!   "自那笔投活以来那颗核一次都没醒过"。
//! - 而**验收门**（`crates/gate/tests/examine.rs`（已删））与看机那两条（`crates/gate/tests/console.rs`（已删）
//!   的 `fast` / `probe` 档）一直是**关着 icount** 跑的。压测两台（`crates/gate/tests/stress.rs`（已删）/
//!   `tests/load.rs`）与 `tests/soak.rs` 没关 ⇒ 当时两边读数**不可比**。
//!
//! **今天这一格已经没了**：门统一在 `gate::run` 里给**每一台**机器置 `QEMU_ICOUNT=""`
//! （`crates/gate/src/lib.rs`（已删）那段"照实记（`QEMU_ICOUNT` 置空）"），不再靠各台自己声明——
//! 原先是三份脚本各自显式置空（`scripts/{stress,soak,load}.sh`，**那批脚本已删**，判据搬进
//! `crates/gate/tests/`（已删））。对齐之后，同一颗 ELF：
//!
...
//! `fetch.rs`）⇒ 该换算成立，本台子的档距假设（下面的 `DELAY_MAX_US`）应当按 `sleep(1 ms)`
