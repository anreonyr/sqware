...
//!
//! **今天与它配对的是 shell**：`crates/gate/tests/console.rs` / `probe.sh` 从控制台上点名跑它
//! （`churn 400 1 1`）。旧版台主（`rig`）曾按"造 → 放行 → 空转到某一刻 → 杀 → 判"反复跑它、
//! 逐档扫那一刻——那一档已经换成 `hang`（见 `rig.rs` 头注），本程序留在清单里备查。
//!
//! **特权级由清单定**：本域是 U 态（`kernel/build.rs 的清单（`PRODUCTS` / `PROBES` / `RIGS`，按场景选表）`）——压测要的是挂起/在台
