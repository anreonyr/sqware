...
**第 12 轮：名册的"活过的任务"账（已修，churn 下 1059 → 1）**

排查过程中量出一笔**真泄漏**（与本条那个 152 B 的关机尾巴同源同形，但它是**会话期
累积**的）：`enlist` 在任务产生时插一条 `Weak<Task>`，而条目此前**只由 `rip`（关机）
清空** ⇒ "开机以来活过的每个任务"都把它那份 `ArcInner<Task>` 外壳扣到关机。名册本身
还是 `HashMap<usize, Weak<Task>>`，容量也随条目数增长。

```
harden 档，churn 150 2 2（读 probe_messenger 在 rip 之前的规模）
  改前：roster 1059 alive 0   ← 1059 个外壳（≈161 KB）+ 表容量
  改后：roster    1 alive 0
```

修法：`scheduler::core::prune_dead()`（`retain(strong_count > 0)`），在
`messenger::bury` 每回收一个躯壳时调一次。**死条目是纯垃圾**：`muster` 的契约本就是
"`Some` 升不起来 = 已消失"，摘掉它不改变任何读取方的语义（`Join`/`Hatch` 照旧拒绝、
`descends` 照旧 false、`snap` 照旧跳过）。代价 `O(在世条数)`。

**仍未定案的**：关机审计那条 `leak: task 1` 的**持有者**。排查已排除：名册 / 票根 /
躯壳 / 站点（`rip` 清空，post-rip 实测 0）、全部活跃团队的 `Team.tasks` 与 `Team.sire`
（逐团队普查 0）、静态 `Weak<Task>`（信标第一版自造过一个 ⇒ 每轮复现，改成 id 后归零）。
"扫内存找持有者"这条路已证明**会被陈旧字节骗**：首轮 3 处命中里，第 3 处是名册
**已清空但未清零**的表缓冲里的指针字节（故判据必须配 post-rip 空表检验）。
下一步：既然命名容器全部排除，就剩"**弱引用对象本身被泄漏**"这一族（某个
`Vec<Weak<Task>>` / `HashMap` 缓冲**未 drop 元素就释放**）。判据可以做成
"弱引用收支"：把 5 个创建点包一层带 `Drop` 计数的 `TaskWeak`，与各容器现存条数对账 ——
收支不等即证明"有弱引用对象的析构没跑"。

**第 13 轮：弱引用收支账 —— "下一步"落地，机制与出身都拿到了**

上一节末尾写的下一步（"把创建点包一层带 `Drop` 计数的 `TaskWeak`，与容器现存条数对账"）
本轮做完了，而且比预期多拿到两件东西：**因果链的焊死**与**出身的点名**。

**① 账本身**（`kernel/src/work/unit/weak.rs`）：`TaskWeak` 是 `#[repr(C)]` 的
`{w: Weak<Task>, id, site}`，仓内一切 `Weak<Task>` 只经它产生 ⇒ 每次**生**（构造/抄件）
与每次**亡**（析构）各记一笔，生的时候把**出身**写进定长槽位（哪个点、哪颗核、出生时的
trap 栈指针）。**`Clone` 有意不实现**：每一次抄件都必须在调用点写明"这枚是从哪儿出去的"
（`muster` 还是快照），编译器替这条纪律站岗。判据是一条纯算术：

```
存 = 生 − 亡 == 0        （没有一枚弱引用对象被丢在"析构不会跑"的地方）
```

健康轮实测（harden+audit，run5 那 13 条命令）：

```
[audit] 弱引用收支：生 200613 亡 200613 **存 0**｜逐点 生/亡：名册 13/13 ｜票根 6574/6574
        ｜团队簿记 13/13 ｜血亲 5/5 ｜抄件·muster 192242/192242 ｜抄件·快照 1766/1766 ｜
[audit]   存活清单：空（每一枚弱引用的析构都跑到了）
```

⇒ **零误报**（20 万枚弱引用逐点配平）。

**② 正对照（一次性、不入库）**：在 `Join` 入口把一枚 `muster` 抄件 `mem::forget` 一次
（`mem::forget` 等价于"析构永不执行"），同一轮里同时出现：

```
[audit] 弱引用收支：生 37277 亡 37276 **存 1**｜… 抄件·muster 29184/29183 ｜
[audit]   存活 #1 id=522 出身=抄件·muster hart=0 sp=0xffffffffeae0f780 指着 strong=0 ← 就是它扣着外壳
[audit] leak: task 1
```

⇒ **"有一枚弱引用对象的析构没跑" ⇔ `leak: task 1`（`strong 0 weak 1`）**，这条因果链
此前只是推论，现在两侧都在同一份日志里。顺带钉住账的灵敏度：**1 枚 / 二十万枚**。

**③ 真机实测（A/B 二进制：带仪器、去掉名册清理）**：一次**正常收尾**的运行里出现

```
[audit] 弱引用收支：生 28466 亡 28465 **存 1**｜… 抄件·muster 20216/20215 ｜
[audit]   存活 #1 id=28233 出身=抄件·muster hart=3 sp=0xffffffffeae3f750 指着 strong=3
```

这一轮 `leak` 计数是 0 —— 因为那枚弱引用当时**指着的任务还活着**（`strong 3`）。
按正对照，只要它指的是一个已经消失的任务，读数就正好是 `strong 0 weak 1` + `leak: task 1`。
**"偶尔有一枚弱引用被丢下"这件事在本机上是真的**，不再依赖历史那一次偶发。

**④ 出身把范围收到一条路**：`抄件·muster` —— `muster()` 抄出来的临时弱引用**只活在
调用方的栈帧里**。按设计内核在切换前会把栈展开（`block` 的落点是"把下一个 `pa` 还给
trap 层"），所以"抄件没被析构"只剩一种可能：**那一叠帧被冻住了** —— 它所在这颗核
**在任务上下文里就地停了机**。`conductor::halt()` 是从 `fetch()`/`wait()` 调的，而
`fetch()` 既可能从空闲循环到达，也可能从**任务自己的调用链**到达（`block` 的
`Handoff::Switch(next_pa.unwrap_or_else(run))`）—— 后者一停机，`hooked()` 就在**一叠
再也回不去的任务帧**上跑审计，帧里的 `TaskWeak`/`Arc` 永远不会析构。
这与 §3.4 那条"关机挂住"是**同一处**的两个后果。

**⑤ 本轮新增的两条判据**（都跑在 post-`rip` 的 `probe_teams` 里）：

- `账/帧 一致性：在册记录 N 条，其中所在帧不在手 0 条（应为 0）` —— 块侧对偶于帧侧的
  `residual == 0`：**账说活着的每一块，脚下的帧必须在手**。这是常驻判据（每次关机都跑），
  专门抓"活着就被拆掉"那一类（健康轮实测 40 条在册记录、0 违例）。
- 判泄漏时逐条打 `所在帧在手=` 与 `块池：owner=… 该页在册块=N`（`Meta::used`，由块层
  `alloc/dealloc` 直接维护，与 `Ledger` **互相独立**）—— `used == 0` 即**假泄漏**
  （块早被归还，读到的 `strong/weak` 是陈旧字节）。

**⑥ 取证扫描必须先问页表**：`locate_weak` 第一版按"窗口 + 保留区清单"裸扫，把一次
"存 1"的报告升级成了 `kernel page fault`（`trap.rs:281`），取证输出全丢。改为按页问
`Space::translate`（`page_mapped`）后才安全。同类事故这是**第二次**（`who_holds` 曾扫到
`0x87f90000`）—— 记进 §5：**诊断工具不许因为自己要读几页内存就把整机打死**。

**⑦ 之前那些"探针"已撤（同轮清理）**：持有者点名之后，为**定位**它而造的一次性取证
器械全部删除 —— `who_holds`（裸扫 DRAM 找"谁还指着这个块"）、`locate_weak` / `Where` /
`page_mapped` / `owner_of`（按 `{id, site}` 指纹找弱引用对象现在躺在哪）、
`block::page_meta` / `Tally::read_pa`（"账说在册 ⇒ 池簿记那一页得 >0"的交叉核对）、
`dump_records` 里那两列 `所在帧在手` / `块池：owner=… 在册块=N`、以及 `probe_ledger_frames`
（"账/帧 一致性"，一次都没响过）。**留下的只有判据**：关机那三行
（`post-rip 空表检验` / `弱引用收支` / `挂起自检`）与 `check_baseline` 自己那条泄漏判词。

**⑦ 持有者点名 + 修掉（本轮）**：上一条那个"帧被冻住"的推断，本轮变成了**可判定**的一条
判据，而且**不依赖偶发**。

判据（`weak::check_block_heldout`，`block` 每次挂起前问一次）：本核此刻还活着、且出身是
**抄件**（`Muster` / `Snapshot`）的弱引用必然活在**本核正压着的栈帧**里 —— 抄件不落任何
容器，已返回的帧里的抄件早就析构了。于是"挂起前它应为 0"是一条纯算术判据。

改前实测（同一个 13 条命令的会话，正常停机）：

```
[audit] 挂起自检：**4 次挂起时本核栈上仍有抄件**（出身：抄件·muster 抄件·muster 抄件·muster 抄件·muster）
```

⇒ `dispatch` 的 `Join` 臂里那枚 `muster` 抄件（`target: TaskWeak`）**真的跨过了挂起** ——
它正是账面「出身=抄件·muster」指的那个持有者：本核一旦被弃栈（被别核判死、或收尾时就地
冻住），它的 `Drop` 永不执行，目标任务的 `ArcInner` 外壳（152 B）被一枚**永远活着**的弱
引用扣住，读数就是 `strong 0 weak 1`。

修法（同一条纪律的延伸，见 `block` 头注原有的"跨挂起不得持强引用"）：

1. `Join` 臂在 `messenger::join(...)` **之前**放掉那枚抄件（`drop(target)`）—— 与紧邻的
   `drop(ident)` 同一条理由，只是弱引用钉的是外壳而不是载荷。
2. 顺手把**弱引用**也按值移交，不留副本：`block`/`wait` 收 `Weak<Life>` 按值、站点成为
   唯一持有者（`site.life = life;`，不再是 `clone()`）；`join` 先拆 `TaskLife` 再移交；
   Wait 臂与 `hole::wait` 都是**移进**而不是借用。此前 `park`/`wait`/`join` 的帧里各留
   一枚副本（钉住的是零尺寸 `Life` 的 16 B 外壳，看不见，但同一类账）。

改后实测（同一会话）：

```
[audit] 挂起自检：每次挂起时本核栈上都没有抄件（跨挂起的引用 = 0）
```

⇒ **同一个判据、同一条命令、修前 4 次 / 修后 0 次** —— 这是"点名后的实测对照"，而且
不再需要等那 1/8 的偶发。附带效果：`leak: task 1` 这一类**从构造上不再可能由该路径产生**
（挂起跨过的调用链上已经没有任何需要析构的引用）。

**仍然存在的一条**（未修，记一笔）：`fetch`/`wait` 就地空转仍可能发生在**任务自己的调用
链**里（`block` 的 `Handoff::Switch(next_pa.unwrap_or_else(run))`），于是本核若在那里等到
`done()` 就地停机，**这一叠帧**（现在只剩标量与已移交的空壳）仍会被冻住。当前它不再钉住
任何任务外壳（引用已全部交出去），但这条"停机深度"的形状本身没有变；要根治得让
`fetch`/`wait`/`halt` 只从 trap 层深度进入（`Handoff` 加一态），那是调度/陷阱边界的改动，
本轮未动。§3.4 的挂住仍是这条路上的另一个后果。

### 3.4 关机偶发**挂住**（harden 档约 1/10 轮）
...
**第 13 轮：抓到一次真挂住，现场与信标的盲区**

猎捕脚本的一次运行（harden+audit，run5 那 13 条命令）真挂住了，`exit` 之后走到

```
bye
root: session over, shutting down
[ERROR] no map for user page fault: Load at VA(0x23001), pc=0x1024c
user fault killed: tid=7 cause=13 stval=0x23001
[audit] space asid 1 retired 2 user-heap records
… （asid 6） …
```

然后**永不停机**。gdb（`trace/hang-1002-*.log.gdb`，ELF 与那一轮同源，可符号化）现场：

```
CPU#0/#1/#3 [halted]  pc=0x80223b36  → fetch::wait  fetch.rs:142   （WFI 循环里）
CPU#2       [running] pc=0xfffffffffffff0de sp=0x84197000 → **trampoline 页**（TRAMPOLINE+0xde）
```

两个结论：

* **挂住发生在关机路径的"空间退役/清退"阶段**：三核已在空闲 WFI，一核在 trampoline
  里打转（RFENCE/清退那一类握手），而屏障等不到它 ⇒ 谁也停不了机。
* **两行 `[stop]` 信标一个字都没出** —— 这是信标的一个**盲区**（此前没意识到）：
  `wait()` 睡下前把 timer arm 到 `WFI_FAR`（无待唤醒 tock 时 = "永远"），于是它**不再
  回到循环顶部**，`beacon::idle` 自然不会再被调到。⇒"收尾期停滞就发声"这条设计在
  **长睡**这一形态下不成立。修法方向（未做，记一笔）：收尾期（`done()` 已成立或根已
  `Reaped`）把 WFI 的 delta 钳到有界值（例如 100 ms），让信标每拍都能说话。

**同一形态第二次**（`trace/hang-1003-*.log.gdb`，同样在关机走到 `space asid N retired`
之后）：三核 `pc=fetch.rs:142`（WFI），一核 `pc=trap.rs:147`（**陷阱入口**）。两次合起来
指向同一个形状：**一核在"清退/握手"里转、其余核睡在 WFI 不应答** —— 与
`asid::shootdown` 的 RFENCE 握手（应答方必须在 WFI 里被 IPI 唤醒）最吻合。若成立，
挂住与"信标发不出声"是同一个根：**睡下的核既不应答握手、也不再回到信标循环**。
本条仍是**假设**，未经判据验证（未动代码）。

### 3.5b 32 M + `churn 16 4 4` 稳定卡死 —— **已定位并修复**（用户报）

**症状（复现）**：`-m 32 -smp 4`，`churn 16 4 4` 后 `exit` ⇒ **永不停机**，四个核全睡在
`fetch::wait` 的 WFI（gdb 现场：四核 PC 全在 `fetch.rs:142`）。信标（本轮修好之后）报：

```
churn: spawn failed with code -1 after 1/4 at round 1
[stop] hart 2 空闲等待：PUSHED=66 REAPED=62（差 4）
```

**根因（两处，都在"任务消失却没记账"这条缝上）**：

1. **`Team.held` 是单槽**（`SpinLock<Option<Arc<Task>>>`），而 `Spawn` 来自**同域的任何
   线程**：两个线程各自 `Spawn`→`Hatch` 时，后一个 `Spawn` **覆盖**前一个未放行的任务
   —— 被覆盖者的外壳当场掉，它的 `Hatch` 只能答 `Denied(-1)`（名册里那枚弱引用升不起来）
   ⇒ 就是上面那行 `-1`。
2. 被覆盖（以及其它"外壳先掉"的形态）的任务**只记过 `PUSHED`、永远等不到 `REAPED`**
   ⇒ `conductor::done()` 恒假 ⇒ 全机在空闲里再也停不下来。

**修法**：

* `Team.held`：单槽 → **表**（`Vec<Arc<Task>>`），放行/他杀一律**按身份摘除**
  （`release_held`，`Arc::ptr_eq`）——"拿走唯一那一个、不是它再放回去"的舞蹈在并发下
  必然出错，直接取消这个形状。
* `Task::drop`：外壳被放掉时若**没走过 `reap`**（`tag() != Reaped`）就补一笔退出账
  （`conductor::exit`）。正规路径 `bury` 已计过，故不会重复；其余形态都还没记过账。
  这条是"死角兜底"：任何将来新出现的"任务消失"路径都不会再把停机判据钉死。

**验收（同一个复现，三档对照）**：

```text
改前： churn: spawn failed with code -1 after 1/4 at round 1 … [stop] PUSHED=66 REAPED=62（差 4）⇒ 永不 halt
改后： churn: 4  rounds x depth 4 x fan 4 done in 9298 ms … bye … task: all tasks exited, system halted
       churn: 16 rounds x depth 4 x fan 4 done in 31625 ms   ← 16 轮 × 341 个任务全跑完，不再有 -1
```

**顺带修好的（否则这条根本看不见）**：信标在收尾期说不了话 —— `wait()` 把 WFI 的拍子
arm 到 `WFI_FAR`（"永远"）之后**不再回到循环顶部**，`beacon::idle` 自然再也调不到。现在
**收尾期**（根任务已 `Reaped`/`Doomed`/已消失）把拍子钳到 `BEACON_TICK`（25 ms @10 MHz
timebase），正常期照旧长睡。上面那行 `PUSHED=66 REAPED=62` 就是它给的。

**仍未修（新发现，记一笔）**：32 M 下跑**重** churn（16 轮 × 341 任务）之后，**控制台输入
不再被处理**（`exit` 打了没有回显、shell 停在提示符上）—— 轻量 churn（4 轮）之后输入正常、
停机正常。这是**另一条**（输入/控制台路径在重压之后失活），与本节那条记账缝无关；
表现为"看起来也像卡死"，故下一轮优先查它。
...
**探针扫内存之前必须先问页表。** 同一个坑踩了两次（`who_holds` 扫到 `0x87f90000`；
`locate_weak` 扫到 `0x87f93000` 那一带），两次的后果都是**把一次可读的报告升级成
`kernel page fault`**（`trap.rs:281`）——第一次丢的是泄漏取证，第二次丢的是
"弱引用收支"刚报出的那条线索。教训不是"窗口算错了"，而是**声明与事实是两回事**：
"帧池窗口 − 保留区清单"是**声明**，页表才是**事实**。按页 `Space::translate` 问一句、
问不到就跳过（`audit::page_mapped`），代价是每页一次页表走。
