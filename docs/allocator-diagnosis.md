...
**仍未修（本轮到此为止）**：停机必须发生在**没有活任务帧压在栈上**的深度。两条候选修法：

1. `block` 不就地取活：把"本核无后继"变成第三态还回 trap 层，由**那里**的深度取活/停机
   （`Handoff` 加一态 + 4 个调用点 + `dispatch`/`trap_handler` 各一处）。语义最干净，改动面也最大。
2. 停机前先确认本核身份槽**已不是 `Live`**（即不在任何任务的帧里），否则先 shed 身份、
   把那一叠帧按"无后继"交回 trap 层再停。

两条都改调度/陷阱边界，本轮**未动**（没有判据能证明它们不破坏既有语义，宁可留着这条已
点名的账）。A/B 的弱信号记一笔：去掉名册清理的二进制 **2 轮里 1 轮**报"存 1"，
带清理的 **8 轮 0 次** —— 样本太小，不足以下结论。
