...
（`console::session::grant`）。为什么必须有这一格，见 §5 第 1 步。
...
1. **Open**：客户端 `console::session::open` —— ① 自建一枚**私有的握手孔**并把 `R|W` 副本授给服务；
   ② 把首帧 `Open` 推给请求孔，**握手孔的句柄写在帧首那 8 字节地址槽里**；
   ③ 在**自己那枚**握手孔上有界等回执。服务端收到 `Open`：按推者开的**回信孔**（`session::grant`）
   把句柄连同认领号推回握手孔，再把 `Ok{client}` 推进刚开的那枚回信孔 ⇒ `session::open` 一次收齐
   ⇒ 客户端拿到会话。**开会话这一段是 console 自己的**（私有孔 + 认领号 + 回信孔的交接），
   机制只给"两枚孔配成一对"（`Port`）。
...
| 不变量 | 违反会怎样 | 谁守着 |
|---|---|---|
| 回信孔由**服务端**开 | 服务被打死那扇门不死 ⇒ 客户端永久挂在 `Busy` 上（「对端还没回」与「对端已经没了」不可区分） | `session::grant` + `gate::doom` 的寿命边 |
| **握手回执走本端私有的孔**，不走请求孔 | 请求孔是一枚单槽信箱、两个方向的消费者：回执会被**服务自己的请求循环**吸回去（实测 50% 开不成） | `session::open` 自建 + 地址槽递出去；`State::open` 只推给它 |
...
| 写缓冲 | **按行 / 128 字节合并**（`Terminal::CAP` ＝ `console::LINE`） | 一次 flush 落成**一条**消息：短串不必一次往返，而超过协议上界的串会被劈成两条、让别的写者插进来（实测：27 字节的报到被劈成 `…reconnect` + 别人的一句 + `ed`） |
...
- **实测逼出来的六处性能修正**（每一处都曾让门判 0/3 或让用户当场看到）：①每条输出开关一次
  会话 → 4 次往返；②一条消息一往返 → 按行攒（现在是 128 B 一条，见 §3）；③服务每 1 ms 轮询 → 「没事就阻塞」+ 两档等待；
