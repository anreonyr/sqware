...
9. **本域不认识自己的线号**（**实现期**，`driver.md` §12 甲）：登记时只报**设备名字**，线号由
   ...
   **设备名也不再硬编码**：它由 root 按配给转达（同一通道的第三件）。
10. **登记失败即降级成有界轮询**（`IRQ_WAIT_MS`），但**降级有一个可观测的代价**：门里那条
   `plic: line 10 delivered` 只会由中断路径产生 ⇒ 降级那轮门会判红。失败时本域还会**封印**
   自己那两枚孔（会话 + 回执）：驱动那侧若已经收下它们，下一次投递会拿到 `Dead` ⇒ 收线
   （否则驱动的 `push` 会阻塞在一条没人排空的槽上）。
