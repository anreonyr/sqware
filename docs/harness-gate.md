...
- **服务那一侧搬进 SUT**：本轮只把**账**修对（谁判什么、判到没有）。→ **下一刀做了，见 §8**；
  而且量完之后发现这一条里"服务台打的多是叙事"那句**是错的**（§8.1）。
- **`member:` 那六条为什么走宿主断言、不写成用例**：它们是**一条读数一行**（不是一族），宿主
  `need` 一行就够；用例那一半的价值在"一族多例 + 名字即结论"，吃到它的是探针那种 19 个计数器的台。
  → **下一刀：这一句一半对一半错**——它们确实是一行一条，但"名字即结论"那一半的价值对**脚本型**
  的服务台同样成立（§8.4 那 47 例就是那么搬的）。


## 8 · 服务台搬进 SUT（用户裁定）

**上一刀留的那句话**：§6 里写着"服务台打的多是**叙事**，硬搬只会把叙事当判据……真要做，得先逐条
回答'这一行**判的是什么**'，那是另一刀"。这一刀就是那一刀，而**第一件事就是把那句话量一遍**——
量完发现**上一刀那个判断是错的**。

### 8.1 账（量出来的，不是估的）

服务侧 **84 行**读数、11 个前缀，宿主一共钉着 **75 条**。逐条读过程序自己的头注（那 16 步 / 14 步
脚本，每步都写着期望）之后：

| 类 | 条数 | 例 |
|---|---|---|
| **可搬**：期望来自程序自己的决定（失败码 / 关系 / 自反 / 幂等 / 第三态 / "真改了"） | **43** | `sleeper: past=2` · `policy: bind(self)=err:denied` · `member: amid(sub,c0)=true` |
| **留宿主**：期望是机器 / 装配的事实 | ~10 | `uart: ier=rx at=0x10000000` · `router: device_count=95 ctx=1` · `root: block n=21 …` |
| **纯前缀**：控制流走到这里而已 | ~22 | `uart: line occupied` · `member: done` · `sleeper: found` |

⇒ 不是"多是叙事"，是**判据住在机器里、判词却写在宿主靶上**。搬它不是新造判据，是**把同一句话
挪到它守的那台机器里**——与探针那一刀同一口径。

### 8.2 一条结构事实：常驻域套不上（这一格改了原计划）

`Suite::run()` 是"**一轮一次**"的协议（打 `N cases` → 逐例 `run`/`ok` → 汇总）。服务侧那 11 台分两类：

- **一次性**：启动期一次算完就退场（那三个 `loop {}` 都是 `seek` 的**有界重试**，不是常驻）
  ⇒ `passer` · `guest` · `sleeper` · `member` · `subject`(→`policy`) · `lodger` ⇒ **能搬**
- **常驻**：读数**每事件一行**、永不收场 ⇒ `echo` · `uart` · `rtc` · `router` ⇒ **套不上**
  （实测一轮里 `uart: rang` 5 行、`router: exhaust` 6 行），硬套就得发明一个"第一次事件时跑一次"
  的机制——那是新设计

⇒ 用户裁定 **甲：只搬那 6 台**。

### 8.3 `Suite` 搬去自己的 crate

`harness` **依赖** `programs`（每个测具一声 `extern crate programs;` 借共享入口）⇒ 十台服务域**用
不了** `harness::cases`。故它往下一层搬成 **`crates/cases`**（谁也不欠谁，只依赖 `runtime`），
`harness/src/lib.rs` 留一句 `pub use cases;` ⇒ **探针那 6 份一个字没改**。

**照实记（为什么不放进 `runtime` 或 `programs`）**：用户早先裁定过"**我希望测试和运行环境分开**"。
放进 `runtime` = 把测试协议搬进运行环境；放进 `programs` = 把测具搬回产品。

### 8.4 落了什么：6 台 · 47 例

| 台 | 例 | 抽样（名字即结论） |
|---|---|---|
| `member` | **23** | `entering_twice_answers_ok` · `leaving_took_out_that_pair_not_the_person` · `the_cursor_is_a_threshold_so_the_next_window_is_empty` |
| `policy` | **13** | `the_root_has_no_sire_and_that_is_not_unknown` · `a_stale_key_is_denied` · `adopting_moves_the_name_and_waiving_puts_it_back` |
| `lodger` | **4** | `the_same_line_twice_is_taken` · `the_failed_attempts_left_no_holes` |
| `guest` | **3** | `the_board_took_my_name` · `that_entry_reached_my_table` |
| `sleeper` | **3** | `arming_the_past_is_refused` · `the_slot_is_already_mine` |
| `passer` | **1** | `the_entry_is_on_the_board` |

**照实记（比宿主那 21 条更强）**：宿主那边是 `need`（**存在一次**就绿），而 `member: amid(me,c0)`
这一形在这台脚本里出现**四趟**（立了 / 入 / 领 / 弃之后）——SUT 里每一趟各判一次，四趟答错任何
一处都点名。

**照实记（关系判据也搬走了）**：`policy: me=` 那三条的"**绑 ≠ 领 = 弃**"原先住在 `soak::verdict`
里（宿主数三行、比两个关系）；那三行是 `subject` 自己打的 ⇒ 现在是它的一例，宿主不再数。

**照实记（三处是"控制流证据"，没搬）**：`sleeper: armed=0`（`fail_to_code(None)` 恒答 0）、
`uart: line occupied`、`member: done` 这类——把它们写成 `assert!` 就是把 `bail` 改个名字。
这一格是写的时候**当场撞上**的：第一版给 `sleeper` 写了 `assert!(armed.is_ok())`，而 `armed`
根本不是 `Result`（它是 `Alarm`）——**编译器替我把那个假判据拦下来了**。

**门那一侧跟着缩**：

| | 搬前 | 搬后 |
|---|---|---|
| `soak` 的 `MARKS` | 118 条 | **87 条**（撤 49 条值判据 + 补 6 条前缀 + 12 条逐台基线） |
| `READINGS` | 17 auto · 10 narrative · 2 manual | **20 auto · 7 narrative · 2 manual** |
| 读数里 `[case]` 行 | 60 | **166** |
| 红的时候报什么 | `缺这几条读数：…`（一次报全） | 那一例的名字 + 两个值 + `文件:行:列`（**一次一例**） |

### 8.5 照实记（变异那一门的账跟着动了三格）

这一刀改动了 6 个服务程序的源码 ⇒ 变异表里有 **3 条**的锚点被挪走（**不是变异失效**，是它落的那
一行换了位置：`member` 那条 `leave` 那一趟、`subject` 那条"弃那一趟改成领根"、`lodger` 那条"表
读数说谎"）。三条都按新源码改写了锚点——**键因此变新、自动回到"没验过"**，已逐条复跑，三条**全红**：

```text
  机器·该出的那一位没出       红  这一条读数还在：^\[case\] member: cases 23 ok 23 fail 0$ ·
                                  每个 `[case] run` 都要有配对的 `ok`（run=62 ok=61）
  机器·弃那一趟改成领根       红  同上（`policy`: run=70 ok=69）
  机器·房客的表那一条读数说谎   红  同上（`lodger`: run=72 ok=71）
```

**75/75 锚点仍逐字唯一**。红在**新机制**上：汇总行缺席 + `pair` 点数——与探针那一刀之后同形。
**照实记（顺手补的一格）**：这三跑当场照出"红在哪"只说得出 `run=62 ok=61`、**说不出是哪一例**
——因为失败那一例的名字在 `Gap.saw` 里，而变异那边只取了 `want`。已改成两半都要。
