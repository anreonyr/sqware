...
`Port::call` 要把回信地址写进报文，就得知道**写在哪个偏移**。上一关把这个数收在
`runtime::core::port` 的一个模块常量里（`ADDRESS_AT = 1`，读作"紧跟首字段"），
...
**裁决**：偏移由**协议**声明（`Duet::ADDRESS_AT`，默认仍是模块常量那个数），机制侧只提供
**带偏移的口**（`put_address_at` / `address_at`）。据此目录协议把地址槽声明在**帧首**
（`wire::ADDRESS_AT = 0`）——信封那一格只能落在载荷最前，帧首因此是唯一两边都成立的位置；
帧形随之定为 `[地址槽 0..8)][op 8][entry 9..17)[名字 17..帧尾]`，`CAP = 48`。

**收侧同一条裁决的另一半**：`Directory::serve` 收的必须是**带真实长度的那一段帧**。载荷缓冲
按上界给、尾部有多余的零，长度不钉住那些零就会被算进名字里。
