...
| 线表（`crates/protocol/src/irq/server.rs`，**核心**：数据 + 判定，不碰寄存器） | `new`（建表：收驱动从树里解出的 `(名字, 线号)`）、`refer`（写属主）、`register`（填实例，只填自己那一行）、`holder`（按**线号**取活实例——两条轴各自的用法）、`retire`（摘实例、行保留） |
| 设备树 → 行（`programs/src/lines.rs`，**驱动**） | `from_tree`（设备树 → 名字 → 线号；三个判据都在树那一侧） |
...
- **第四步（线的权威与收线，§12 甲/②）**：判据分三层，**每一层都有牙**。
  ...
  2. **反证（权威是判据，不是装饰）**：shell 的 `line <名字>`（`prog-shell` 的自检命令之一）
     ...
     **本轮实做**，读数如下（同一份代码，只改 `irq/server.rs::register` 里那两句）：
