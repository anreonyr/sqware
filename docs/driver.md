...
| 8.1.17 | `Reap` 带一句话 | 未裁（只有"域自己的诊断编号"） | `Reap { reason, note: VirtAddr, len }`：内核在入口 `copy_in`（栈上定长 `NOTE_MAX = 128`）并**自己打印**（SBI），panic 现场那句 `file:line:col` 由域侧在栈上拼好（`programs/src/entry.rs`） | **已裁**（用户：**"并进重启那一轮一起做"**）· 这一格的账：**内核侧不做符号解析**（`symbol()` 记为**否**，用户："不需要 symbol"）——`PanicInfo` 的消息与位置都是编译期字面量，**不需要符号表** |
| 8.1.18 | 属主写权的委托 | 未裁（§12 甲只说"属主只能由 root 写"） | 第三个动词 `Delegate{name, who}`：`sire` 把**一个名字**的写权借给它信任的线程（判据从"谁的 id"变成"root 有没有为这个名字委托过你"） | **待裁** · 建议追认：没有它，root 的服务重启就写不了属主（实测 `Err(44)`）——监护线程不是域，驱动那条 `from == sire` 判据它永远答不上；而"放宽成同一个域"在驱动侧**不可表达**（拿不到推者的域） |
| 8.1.19 | `Taken` 的语义 | §3.2.8 写的"旧实例还占着" | **先探持有者存活**（`mail::reserve`），不在了就当场摘空让新人登记 | **待裁** · 建议追认：它把 `Taken` 收紧成"**活实例**占着"，顺手闭合了 §9.14 的悬空实例——代价是登记路径多一次探测（一枚门闩的查表） |
| 8.1.20 | 监护线程怎么"等它死" | §12 只裁了"收线靠一次失败" | **探能力链**（手里那枚指向它的入口副本还在不在，20 ms 一圈），**不是 `Join`** | **待裁** · 建议追认（并记下代价）：`Join` 的授权是**任务粒度**的"它是不是我生的"（`envcall.rs:521-526` 的 `me.heir(target_team)`），而主线程起第一次、监护线程管余生 ⇒ 监护线程**等不了**它（实测 `Denied`）。换成 `Join` 只有一条路：让监护线程当生父（从第一次起全包），代价是 main 要等一条"console 已上线"的报到通道 |
...
14. ~~**"戴着面具死掉"的客户端会留下悬空实例**~~ → **已闭合（本轮）**：`Taken` 现在先探持有者的存活（见 §3.2.9 补记）——**悬空实例当场自愈**，不必等下一次投递。原文保留在下面（它是当时按"收线靠一次失败"这条裁决如实记下的代价）：
    ~~收线的触发点是**一次投递失败**。若客户端在"自己把 `IER` 关掉、PLIC 又恰好没有挂起位"的状态下死掉，就没有下一次投递 ⇒ 实例留在表里。此后 root 把名字交给新实例，新实例拿到的是 `Taken`（诚实，但要走救火路径）。~~
...
- **第五步（服务重启，`docs/root.md` §5.3）**：**收线那条缺口现在有门内判据了**，而且是三档都核：
  1. `console: line <名字> ok` 必须出现**恰好两次**（引导期一枚 + `kill console` 之后 root 重发的一枚）。
     它只在**新实例拿到 `Ok`** 时出现——`Taken` / `NotYours` / `Unclaimed` 都会走降级路径、不打这一行。
     于是"旧实例收干净了"这件事有了直接读数（不必读 PLIC 寄存器，也不必看 enable 位）。
  2. 随后的 `exit` 由**新实例**回显 ⇒ 会话真的活着（服务侧重发 + 客户端重连两半都在）。
  3. `kill console` 那一步的期望串是 `shell: console reconnected`——**客户端那一半**的读数。
  **牙在哪**：把委托（§5.6）去掉 ⇒ 新实例的 `Refer` 被拒 ⇒ 它永远拿不到 `Ok` ⇒ 第一层计数判据当场变 1；把"活实例"判据（§3.2.9 补记）去掉 ⇒ 新实例拿到 `Taken` ⇒ 同样是 1。**两条都是本轮实测踩出来的**（不是设想）。
