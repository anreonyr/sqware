...
| K7 | **关机次序**：`exit()` 之前**先收掉每个子域**（按**名字**，与用户杀服务共用 `collect`），再放本域自己的空间。理由是同一件事的两半：级联管"父死子随"，而本域退场时**自己那页设备内存（UART）也要收回**，两半不是原子事（4 核）⇒ 子域会落在"映射已收、任务还没死"的缝里（§7.6 的实测）。**等它走探的是手里那枚副本，不是 `Join`**——`Join` 是任务粒度的，重发出来的实例只有监护线程等得到（§8.1.20） |
...
6. ~~**关机路径上有一处"设备映射被收回之后又去碰设备"的读**~~ → **已修（乙：先收子域，
   ...
   - **修法（乙）**：`main` 在 `exit()` **之前**把每个子域收一遍，于是"级联扑杀"与"本域放
     ...
     - **按名字收，不按 id**：console 可能被**重发**过（那是另一个 id），而"哪个是当前实
       例"这条账在**目录**那儿。故收尾走 `collect(&dir, name)`——与用户杀服务**同一段代码**
