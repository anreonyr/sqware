...
6. **关机路径上有一处"设备映射被收回之后又去碰设备"的读**——**站点是既有的，但本轮把它从
   偶发撑成了常态（实测，别当成无关）**。`no map for user page fault: Load at VA(0x23001)`：
   `0x23000` 是 UART 那页设备内存，读的是 `IER`（`0x23001`）；符号化落到
   `programs/src/uart.rs:114`（`Uart::read`）← `:104`（`mask_rx`）← `console.rs:258`，
   即 **console 的输入线程"进门关中断"那一步**。
   - **站点不是本轮的**：`docs/allocator-diagnosis.md` §13 记过一次同址现场
     （`pc=0x1024c`），符号化同样落到 `uart.rs:104/114`，而那次运行的命令序列**没有**重启
     这一步。
   - **窗口是本轮撑开的**：同一套判据、同一台机器——本轮配置（含 `kill console` + 重发）
     4 轮里 **3 轮**出现（tid 14/14/18，pc 全是 `0x10302`）；干净 master 上的对照
     （4 轮默认档、12 步、无重启）**0 轮**出现。两次都在 `bye` →
     `root: session over, shutting down` **之后**，随后仍照常停机
     （`task: all tasks exited, system halted`）。
   - **门的判据不含它**（三档只问"自退 + 无 panic + 步/marker 齐"），故它不改变本轮结论；
     但它也不是"可以忽略"：它说的是**关机期有一个缝**。
   - **机制（未证，别当结论）**：设备那页是 root 的所有物（`hand_over` 把 UART 的副本交给
     console），root 退场时要收回/退役它，而"收子域"与"放自己的空间"不是一件原子事（4 核）
     ⇒ console 的输入线程可能在"映射已收、任务还没死"的缝里再读一次设备。谁先谁后
     （放空间 / pole 退役 / 级联扑杀）**没有测**，只测到"缝存在，且重启那一步把它撑得很宽"。
   - 修的方向（**待裁**，两条都别顺手改）：① 让设备窗口的寿命跟着**用它的人**（映射由
     console 自己持有），② 让 root 的退场**先收子域、再放自己的空间**（次序成为判据）。
     ①动的是"设备面的所有权"，②动的是关机次序。
