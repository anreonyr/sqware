...
2. **属主的写权要委托**：重发出来的是**新 task id**，行表的属主得跟着换；而驱动认的是
   ...
   `Err(44)`**（实测）。
3. **`BACK` 那一位不能给**：UART 门闩与建域权授给监护线程时带 `VEST`、**不带 `BACK`**——
   带 `BACK` 的源只能授给 sire 的持有者（`gate::snap::vestable`），而它要授给的正是自己刚生
   的子域 ⇒ 那一授会被 `Denied`（实测：卡在 `hand_over` 的 `Err(19)`）。它不需要 `BACK`：
   它只往下授，不回授。
