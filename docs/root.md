...
| L1 | **线的属主由本域写**（§5.2）：驱动只认"推者是不是它的 `sire`"，故"谁能用哪个名字"这张表由本域一支笔写——**与目录的 `Refer` 同构**（名字不是运行时判定，是表里有没有写你的行） |
| L2 | **名字由本域转达**：名字的账在 boot 的配对块里（本域是它的读者），子域要用就得走本域——机制不新增（一次性孔 + `Pier` 走同一条下行通道），且**次序即正确性**：写属主在前、交名字在后 |
| L3 | **本域用完就放下**驱动入口副本：`reach` 连一次、用完 `release`（本域手里零服务孔，§10） |
| K5 | **监护与重发**（§5.3）：主线程起第一次、**监护线程管余生**（同域线程，一枚 `Kit` 经 `KIT` 交接）；重发走**与首启同一对函数**（`spawn_service` + `wire_console`——各写一遍必然漂）；预算是**政策值**：10 s 窗口内 3 次，`Build` 失败即终止 |
| K6 | **属主写权的委托**：重发出来的是**新 task id**，而行表属主的判据在驱动那侧（"推者是不是我的 `sire`"）。监护线程不是域 ⇒ 加协议第三个动词 `Delegate{name, who}`（`sire` 按名字借出写权）；放宽成"同一个域"**不可表达**（报文只有一个 `from: TaskId`） |
| K7 | **关机次序**：`exit()` 之前**先收掉每个子域**（按**名字**，与用户杀服务共用 `protocol::doom::collect`），再放本域自己的空间。理由是同一件事的两半：级联管"父死子随"，而本域退场时**自己那页设备内存（UART）也要收回**，两半不是原子事（4 核）⇒ 子域会落在"映射已收、任务还没死"的缝里（§7.6 的实测）。**等它走探的是手里那枚副本，不是 `Join`**——`Join` 是任务粒度的，重发出来的实例只有监护线程等得到（§8.1.20） |
...
- **线格式**（`crates/protocol/src/doom/wire.rs`，两端共用一份定义）：`Kill = [op][target 32][ack 8]`
...
- **判据不在本域**：上面①②③三步住 `protocol::doom::server`（`collect` = 收一个名字，
  `serve` = 认动词、认人、给结局），本域的线程只剩装配——开孔、注册、推回执、收场。
  关机收尾（§5.5 K7）与用户杀服务**共用同一个 `collect`**：两件事本来就是同一件。

### 5.2 线的属主与设备名字（`docs/driver.md` §12 甲）

**"谁能用哪个名字"是本域写的**——与 `dispatch` 的 `Refer` 同形：不是运行时判定，而是表里
有没有写你的行。中断驱动的行表按**名字**给属主，`Register` 只能填自己那一行；本域因此要
在两处出手：
...
- **次序是硬要求**：**设备门闩 → `Refer`（写属主）→ 交名字**。客户端一拿到名字就去登记，
  而登记读的正是 ① 写的那一行；两件事走同一条 FIFO 报文队列 ⇒ 先推的一定先被处理。
- **名字的账在 boot 的配对块里**（`(名字, token)`）：本域是它的读者，子域要用就得由本域
  转达，别处没有第二个来源——`console` 因此**不硬编码设备名**，也**不认识线号**。
- **准入仍是能力**（与 §K3 同一条）：`Refer` 只由本域发，客户端拿不到"给自己发线"的入口；
  没被写属主的名字，谁登记都是 `Unclaimed`。
- **失败即停机**：`Refer` 没成（名字与设备树对不上 / 驱动不在）⇒ 打印 + `exit_with(44)`
  ——不致命的版本更糟：症状是"输入静默降级成轮询"，而门里那条 IRQ marker 会消失。
- 本域用完**放下**那枚驱动入口副本（`mail::release`）：**本域手里零服务孔**（§10）这条
  同样适用于这里——它只用一次。

### 5.3 监护与重发（服务从此有"余生"）

在此之前 root 只有两态：**产生**与**收场**。服务一死，世界就跟着结束（`shell` 的会话断了 ⇒ 收场）。
现在多了一条：**一个服务死了，就再产生一个**——而这也是"收线的判据"唯一能立起来的地方
（驱动收没收到线，只有"新实例能不能拿到 `Ok`"看得见）。

```text
主线程   起 console（第一次）→ 交棒（Kit：目录面 + 建域权 + UART + 清单视图）→ 等 shell 死 → 收场
监护线程 等 console 死 → 按预算重发（spawn_service + wire_console，**与第一次同一对函数**）
```

**四条要点**（每条都是被实测逼出来的，不是设想）：

1. **谁生谁等**：`Join` 的授权是**任务粒度**的"它是不是我生的"（`envcall.rs` 的
   `me.heir(target_team)`）⇒ 主线程起第一次、监护线程管余生这个形状下，监护线程**等不了**
   console（实测 `Denied`）。故它用**探能力链**：手里那枚指向它的入口副本没了 = 它走了
   （与 §5.1 的 `doom` 服务同一条机制）。**代价如实记**：一圈 20 ms 的探测，不是纯事件等待。
   换成 `Join` 只有一条路——让监护线程当生父（从第一次起全包），代价是 main 要等一条
   "console 已上线"的报到通道（它才敢起 shell）。
2. **属主的写权要委托**：重发出来的是**新 task id**，行表的属主得跟着换；而驱动认的是
   "推者是不是我的 `sire`"，监护线程答不上（它不是域）。故本域在 `Hatch` 之前亲口把**这一个
   名字**的写权委托给它（`Delegate`，`docs/driver.md` §5.6）。**没有这一句，重发卡在
   `Err(44)`**（实测）。
3. **`CAGE` 那一位不能给**：UART 门闩与建域权授给监护线程时带 `VEST`、**不带 `CAGE`**——
   带 `CAGE` 的源会被**关住**（交出 = 授出方在交出期间不可用它），而 root 留源副本正是为了
   重发时**再配一次**；一旦被关住，这一枚就再也授不出去（实测形状：卡在 `hand_over` 的
   `Err(19)`）。它只往下授、不回授，不需要这一位。
4. **一直不能重启 ⇒ 自然停机**（三层有界，缺一不可）：① 重发**有界**（窗口 10 s 内 3 次，
   活够一窗就重置计数；`Build` 失败是**终局**，不重试）；② shell 的重连**有界**（60 × 50 ms，
   实测重发一次 ~1 s ⇒ 窗口 3 s）；③ **本域的正事不依赖 console**——主线程等的是 shell，故
   console 永不起来也挡不住收场。放弃时打印一行 + 监护线程收场，会话随 shell 一起结束 ⇒
   `task: all tasks exited, system halted`。**不会有"永远重试"的循环**：root 一收场，监护线程
   随本域一起消失。
...
6. ~~**关机路径上有一处"设备映射被收回之后又去碰设备"的读**~~ → **已修（乙：先收子域，
   再放本域自己的空间）**。原文与实测照旧留在下面，修法与对照另起一段。
   ...
   - **窗口是本轮撑开的**：同一套判据、同一台机器——本轮配置（含 `kill console` + 重发）
     ...
     （4 轮默认档、12 步、无重启）**0 轮**出现。三次都在 `bye` →
     `root: session over, shutting down` **之后**，随后仍照常停机。
   - **修法（乙）**：`main` 在 `exit()` **之前**把每个子域收一遍，于是"级联扑杀"与"本域放
     自己的资源"之间的缝没了——孩子先死透，本域才退场。两条细节都是被这个仓自己的判据逼
     出来的：
     - **按名字收，不按 id**：console 可能被**重发**过（那是另一个 id），而"哪个是当前实
       例"这条账在**目录**那儿。故收尾走 `doom::collect(&dir, name)`——与用户杀服务**同一段代码**
       （两件事本来就是同一件：把"这个名字此刻指向的那个域"收掉）。第一版按 id 收（并在本域
       存一份"当前实例号" `CONSOLE_LIVE`）被"一个事实一份账"否掉，改判记在这里。
     - **等它走用的是"探手里那枚副本还在不在"，不是 `Join`**：`Join` 的授权是**任务粒度**
       的（谁生的谁能等，`docs/driver.md` §8.1.20），而重发出来的 console 是**监护线程**生
       的 ⇒ 主线程等不到它（`wait_dead` 会把 `Denied` 当"它已经没了"直接返回，等于没等）。
       `doom::collect` 那条判据没有这个问题，而他杀服务本来就用它。
     - 目录自己**没有名字**（它就是名字的账）⇒ 它按 id 收：本域亲生的，`Join` 等得到。
     - 结局照实说，不假装收干净了：`Ok`/`Dead` 都是"它没了"（`Dead` = `kill console` /
       `kill echo` 那两步早收过它），`Slow` 打一行"已下令、没等到"，`Denied` 打一行"不在
       血缘里"——后者在"本域是全体域的祖先"这个结构下不该出现。
   - **判据**：门新增第 5 条「无未处理 fault」（`no map for user page fault` /
     `user fault killed`，三档都核，与"无 panic"同级）。修前它四轮里三轮成立而门全绿——
     这正是"停机照旧"把缝藏住的方式；立起这条判据后，同样的现场会**当场判红**。
   - **未证的那半**：缝的确切因果（放空间 / pole 退役 / 级联扑杀谁先谁后）**没有测**，乙是
     "按最可疑的次序去堵"。第 1 版记账写的是"与本轮无关"，被上面那张对照表推翻后改判为
     "站点既有、窗口本轮撑开"——照实写在这里。
   - **还剩一条理论上没堵死的**（没测到过）：监护线程若恰在本域置 `STOPPING` **之前**进了
     `restart`，那个新实例可能在本域收完之后才落地 ⇒ 它仍由级联收走，缝理论上还在。窗口极窄
     （要"console 刚死"与"shell 刚退"撞在一起，而门里那两步之间隔着数秒），故不当成已修——
     记在这儿，判据（下一条）会在真发生时当场判红。
