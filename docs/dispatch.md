...
5. ~~**`Join` 可能在退出钩子跑完之前返回**~~ —— **已修**：死亡分两相——`suspend`
   把整棵血缘子树的受害者**先全部停摆**（摘出调度/等待容器，置 `Doomed`），再逐个
   `die`（跑退出钩子 → 置 `Reaped` → 入躯壳队列）。于是 `Reaped` ⇔ 钩子已跑完，
   `Join` 返回真即**收尾完成**（`shell` 的 `cascade`/`reclaim` 已删掉有界等待）。
...
crates/protocol/src/dispatch/
  ...
  wire.rs     线格式：64 字节请求 / 定长回复、6 个动词
  control.rs  父域 ↔ 目录**控制孔**上的两条报文：`Refer`（引荐 / 预约）+ `Referred`
  client.rs   调用方会话：open / connect / call / disconnect
  server.rs   服务侧：注册表（预约行 + 至多一个实例）+ 协议适配 `serve`

（前四块是 `crates/protocol/` 里**四个协议共用的形状**——`console` / `doom` / `irq`
同形；`control.rs` 是本协议多出来的那一块：引荐走的是**另一条孔**（控制孔 / 上行孔），
说话人是父域与目录控制线程。`server.rs` 收多少，按"谁是设备"定，见
