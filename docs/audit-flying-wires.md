...
**残留（未修，记账）**：那 1/6 的失败是**门写不进 FIFO**（`只写出 2/8 条`：guest 正常跑完
`spawn`、照常回到提示符，此后写 fd 报错）。写 FIFO 报 EPIPE 意味着**读端已关**，而 qemu 进程
还活着 ⇒ 是 **qemu 自己不再持读 stdin**（不是 guest 不收）。同一套直连写法在另一批里
24/24 步全过，故这是间歇的宿主/qemu 侧行为，与内核无关。下一次要查的是 qemu 在什么条件下
关掉 stdin（`-nographic` + `-icount auto,sleep=on` + FIFO 的组合），或干脆改用
`-serial pipe:`／`-chardev` 让 guest 的控制台完全不经过进程 stdin。
