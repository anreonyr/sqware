...
**判据 = 树手里有没有门牌**，不另加显式握手信号：那一帧紧跟 principal 起来之后 ⇒ "没有门牌"与
"装配期"**等价**。（理由原先还写了第二条——"装配期那些动作根本不经过 `answer()`"；
**照实记：那一句不成立**——principal 挂门牌走的就是树的门，见下。）

**照实记（这一格被推翻过一次）**：原写"没有门牌时客人请求答 `UNJUDGED`（判不了），**不许**答
`Allow`"，并把"放行"读成"协调服务没配上 ⇒ 没人守门"。实现一直是**放行**，而这个放行**不是
给它的例外**，是**装配期根本不在门禁这条轴上**：principal 挂自己门牌那两手（`part /sys` +
`land /sys/principal`）发生在它的 `serve_tree` 里，而本域**认下它的门牌**与它**拿到身份**
（`derive(ROOT)` + `bind`）都在**那之后**（`service.rs` 的 `assemble` 里那两段）⇒ 那一刻它
**既没有门牌、又还没有身份**。门禁若在那一刻生效，它连自己的门牌都挂不上，整机起不来。
**挡门的是运行期那些客人**——它们全都在 `Hatch` 放行之前拿到了身份。

⇒ 以**这一版**为准。`gate::Blind`（"本域手里还没有门牌"那一格）因此**生产到不了**，
它的读者只剩宿主靶（`protocol-case` 的 `judge` 靶，钉的是"这一格在码这一层永远不是 `0`"）。
...
- [x] 开闸判据 = **树手里有没有门牌**。 ← 已定（§1.3）
- [x] **照实记（后一刀翻案）**：原裁决"没有门牌时客人请求答 `UNJUDGED`、不许答 `Allow`"**不成立**
      ——装配期不在门禁这条轴上（principal 挂自己门牌时既没有门牌、又还没有身份），放行是前提不是
      例外。见 §1.3 那段照实记。 ← 已改
...
- ~~甲档坑（按记号认领、转授失败不答 `OK`、`try_reserve`）——树的内部正确性，另一刀。~~
  ...
  **没做 fail-closed，也没造那台负证客人**——照实记：这条判据**天生量不出一个确定的东西**。
  ...
  ~~**干净的关法是另一刀**：让 `ask_hole` 与入口那一枚也走**有名有姓的泊位**（`Quay::seat`
  那条路已经有同名闸），把"只可能有一枚"从**纪律**变成**构造**。~~
  → **已收**（**小形状**，`operator-rule.md` §7.9）：客侧 `ask_hole` **先找后铸**
  （`session::call::find(本端, 记号)`——那一手已经是"按两格正判据找我表里那一枚"），于是
  **同一张表里再也生不出第二枚问话孔**；真机读数 `ask_same=1`（第二次要回来的是同一枚）且
  全程**没有** `operator: two asks`。代价为零：**签名与二十处调用点一个字没动**（若走
  `Quay::seat` 那一版，二十处调用点的签名都要跟着改）。**"入口那一枚"那一半照实记不成立**：
  `find_face` 的头注写着"多枚**正常**（副本共享 `opened_by`：`land` 交一枚、门禁交一枚）"，
  而且 `echo` 就**故意**铸两枚入口（板一枚、树一枚）——那一半不是"纪律被破"，是设计。
  **这一格量出来的另一半更值钱**：两面的问话孔原先刻的是**同一个记号** `ask`
...
上面每一条都停在三种状态之一。把它们分开写，是因为**"不在这一刀里"不等于"待办"**——而一份
分不清这两样的清单，会让人一遍遍重开已经决过的事。

**一、已收**（划掉的那些）：门禁装上与三格裁决 · 落牌不绑身份走不了 · 甲档坑（三处）·
规则的形状（四条判据各有了正负证）· 深度（号就是下标）· 主人退场后可接手 · 问话孔的构造 ·
**规矩里的号从哪来**（第五格 `Opens`）。

**二、已决的边界**（不是待办，理由都在正文里，改主意要先推翻理由）：

| 边界 | 理由落在哪 |
|---|---|
| **树没有死亡道**（板那条 `gone-<名字>` 树一条都不认领） | `operator/mod.rs`："树这一侧不推任何东西给装配者"；替代是**惰性剔死**（`find` 那一问探一次）+ 账对真相（`fresh`）。要真做，就得造一个**每次死一位客人就扫一遍全树**的机制——而今天没有客人要它 |
| `part` 顶掉 `Tile`（静默换绑） | §1.5 的窄口子；今天 `/sys`、`/device` 一直是 `Pane`。**有判据钉着现状** |
| 别名（同一枚 Pie 挂两个名） | 树是"名字 → 一枚句柄"的目录，**不查重**。**有判据钉着现状** |
| 读那一侧公开（`seek` / `list` / `name` 不判） | `operator-rule.md` §1：规矩挡的是**钥匙**，不是**目录** |
| 重启即失（账在服务侧内存里） | 今天没有重启，退出即关机 |
| `opens` 不上线 | 今天没有客人要读"这一格是谁的" |
| 规矩绑在"一次挂载"上（号不重用） | `operator-rule.md` §7.7；**有读数**（`gone_door=9`） |
| `UNJUDGED` 的"身份服务不答"那一条因 | 真机上要量它得把整机拆掉；宿主台那一条是它唯一的读数 |

**三、缺客人的机制**（做过一半、或者**不该在没有真客人时做**）：

- **持树者公平**（`operator-slot.md` §5.4）：真机量过——一台客人连打几百手同步往返，会把别人的
  1 秒期限挤过期（`pname=- trim=7 op=7 seq=1`）。修法（轮转 / 配额 / 让手）都要新机制，而
  **那位聊天型客人已经被请出装配单**（它进 PLAN 的四次尝试全红，见 `operator-slot.md` §6）。
  按"**新机制要有读数**"：先有一位真客人，再动它。
- **`assemble.rs` 的名字与住处**（`programs/src/driver/assemble.rs` 头注）：它住在 `driver` 这一族，
  而 U 态的 `lodger` 反向 `use` 了它——要不要跟着"领门闩的域"这个更宽的口径挪一次。
  这是个**命名**问题，不是缺陷；挪它的收益是"住处与口径一致"，代价是四个文件的 bin 路径一起动。
